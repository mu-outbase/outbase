/* OUTBASE Walk v214: dedicated walk UI + safe result transition */
(function () {
  'use strict';
  let voiceRecorder = null;
  let voiceChunks = [];

  function api() { return window.OUTBASE_LIFE_V214; }
  function $(id) { return api().$(id); }
  function events() { window.outbaseWalkEventsV214 = window.outbaseWalkEventsV214 || []; return window.outbaseWalkEventsV214; }
  function videos() { window.outbaseWalkVideosV214 = window.outbaseWalkVideosV214 || []; return window.outbaseWalkVideosV214; }
  ...
  function hideLegacyWalkCards() {
    ...
    walk.querySelectorAll(':scope > .card').forEach((card, index) => {
      ...
      card.classList.add('walk214-hide');
    });
    walk.querySelectorAll('.life214-nav').forEach(nav => nav.remove());
  }
  function applyWalkMode(options = {}) {
    ...
    if (!panel) {
      ...
      panel.className = 'card walk214-card';
    ...
    api().markReady();
    if (options.scrollTop === true) window.scrollTo(0,0);
  }
  function buildPanel() {
    return `<div class="walk214-head"><div><b style="font-size:22px">散歩で使うものだけ</b><div class="walk214-status">取込・更新・文字追加・下ナビは出しません。</div></div><span class="walk214-badge">現地用</span></div>
      <div class="walk214-grid"><button class="walk214-btn" data-action="photo"><span>📷</span>写真</button><button class="walk214-btn" data-action="video"><span>🎥</span>動画</button><button class="walk214-btn walk214-primary" data-action="voice"><span>🎤</span>長押し音声</button><button class="walk214-btn" data-action="memo"><span>📝</span>メモ</button><button class="walk214-btn" data-action="friend"><span>🐶</span>犬友</button><button class="walk214-btn" data-action="spot"><span>📍</span>スポット</button></div>
      <button class="walk214-btn walk214-end" data-action="finish"><span>■</span>散歩終了</button>
      <details class="walk214-detail"><summary>詳しい記録を開く</summary><div class="walk214-grid" style="margin-top:10px"><button class="walk214-btn" data-action="poop">💩 うんち</button><button class="walk214-btn" data-action="pee">💧 おしっこ</button><button class="walk214-btn" data-action="rest">休憩</button><button class="walk214-btn" data-action="carry">抱っこ</button><button class="walk214-btn" data-action="danger">危険</button><button class="walk214-btn" data-action="water">水分</button></div></details>
      <button class="walk214-home" data-action="home">ホームへ戻る（散歩は継続）</button><div id="walk214Status" class="walk214-status">音声は長押し、離すと保存。</div><div id="walk214Recent" class="walk214-status"></div>`;
  }
  ...
  function setStatus(text) { const el = $('walk214Status'); if (el) el.textContent = text; }
  function updateWalkStatus() {
    const recent = $('walk214Recent');
    if (!recent) return;
  ...
  async function finishWalkSafely() {
    try {
      ...
      const finalize = async gps => {
        ...
        currentSession = null;
        window.outbaseWalkEventsV214 = [];
        window.outbaseWalkVideosV214 = [];
        showResult(record);
      };
  ...
  function showResult(record) {
    ...
    p.className = 'page';
    p.innerHTML = `<section class="life214-card life214-hero"><div class="life214-kicker">WALK RESULT</div><div class="life214-title">コタ散歩を保存しました</div><div class="life214-sub">結果はここ、散歩履歴、詳細から確認できます。</div></section>
      <section class="life214-card result214-grid"><div class="result214-mini"><div class="result214-label">時間</div><div class="result214-value">${api().esc(record.walk.time)}</div></div><div class="result214-mini"><div class="result214-label">距離</div><div class="result214-value">${api().esc(record.walk.distanceKm)}</div></div><div class="result214-mini"><div class="result214-label">GPS</div><div class="result214-value">${record.summary.validGpsPointCount}件</div></div><div class="result214-mini"><div class="result214-label">写真/動画/音声/メモ</div><div class="result214-value">${record.summary.photoCount}/${record.summary.videoCount}/${record.summary.audioCount}/${record.summary.noteCount}</div></div></section>
      <section class="life214-card result214-actions"><button class="result214-btn result214-primary" onclick="showDetail&&showDetail('${api().esc(record.id)}')">詳細を見る</button><button class="result214-btn" onclick="showWalkHistoryPage&&showWalkHistoryPage()">散歩履歴を開く</button><button class="result214-btn" onclick="showOutbaseLifePageV214('home')">ホームへ戻る</button></section>`;
    api().showOnly(api().resultPageId);
    api().markReady();
    window.scrollTo(0,0);
  }
  function patch() {
    if (typeof startWalk === 'function' && !startWalk.__v214) {
      const original = startWalk;
      window.startWalk = function () {
        window.outbaseWalkEventsV214 = [];
        window.outbaseWalkVideosV214 = [];
        const result = original.apply(this, arguments);
        setTimeout(() => applyWalkMode({ scrollTop:true }), 80);
        setTimeout(() => applyWalkMode({ scrollTop:false }), 400);
        return result;
      };
      window.startWalk.__v214 = true;
    }
    if (typeof restoreWalkSession === 'function' && !restoreWalkSession.__v214) {
      ...
      window.restoreWalkSession.__v214 = true;
    }
    if (typeof finishWalk === 'function' && !finishWalk.__v214) {
      window.finishWalk = finishWalkSafely;
      window.finishWalk.__v214 = true;
    }
    if (typeof showPage === 'function' && !showPage.__v214) {
      const original = showPage;
      window.showPage = function (id) {
        if (id === 'homePage') { api().showLife('home'); return; }
        const result = original.apply(this, arguments);
        api().setRoute(id);
        if (id === 'walkPage') setTimeout(() => applyWalkMode({ scrollTop:true }), 80);
        else setTimeout(() => api().enforceOnly(id), 30);
        return result;
      };
      window.showPage.__v214 = true;
    }
    if (typeof backToHome === 'function' && !backToHome.__v214) {
      window.backToHome = function () { api().showLife('home'); };
      window.backToHome.__v214 = true;
    }
    if (typeof restorePendingSession === 'function' && !restorePendingSession.__v214) {
      const original = restorePendingSession;
      window.restorePendingSession = async function () {
        const result = await original.apply(this, arguments);
        setTimeout(() => { if (api().isWalkActive()) applyWalkMode({ scrollTop:true }); }, 120);
        return result;
      };
      window.restorePendingSession.__v214 = true;
    }
    if (typeof discardPendingSession === 'function' && !discardPendingSession.__v214) {
      ...
      window.discardPendingSession.__v214 = true;
    }
  }
  function monitor() {
    const walk = $('walkPage');
  ...
  window.returnToActiveWalkV214 = () => applyWalkMode({ scrollTop:true });
  window.returnToActiveWalkV213 = () => applyWalkMode({ scrollTop:true });
  window.returnToActiveWalkV212 = () => applyWalkMode({ scrollTop:true });
  window.returnToActiveWalkV211 = () => applyWalkMode({ scrollTop:true });
