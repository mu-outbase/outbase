...
(() => {
  ...
  function renderMonth(){
    ...
    const cellMarkup=cells.map(d=>{
      const k=dateKey(d),rows=eventsForDay(k);
      const singles=rows.filter(e=>!isMulti(e));
      const multi=rows.find(isMulti);
      let segment='';
      if(multi){
        const start=k===eventStartDay(multi),end=k===eventEndDay(multi);
        const cls=start&&end?'single':start?'start':end?'end':'middle';
        segment=`<div class="cell-multiday"><span class="segment ${cls}">${start?esc(multi.title):''}</span></div>`;
      }else{
        segment='<div class="cell-multiday"></div>';
      }
      return `<button class="day-cell ${d.getMonth()!==cur?'outside':''} ${k===state.selected?'selected':''} ${k===today?'today':''}" data-date="${k}">
        <strong>${d.getDate()}</strong>
        ${segment}
        <div class="month-events">
          ${singles.slice(0,2).map(e=>`<span class="month-event tone-${calendarById(e.calendarId).tone}">${esc(e.title)}</span>`).join('')}
          ${singles.length>2?`<span class="month-more">ほか${singles.length-2}件</span>`:''}
        </div>
      </button>`;
    }).join('');
    return `<section class="calendar-card"><div class="weekdays">${[...'日月火水木金土'].map(x=>`<span>${x}</span>`).join('')}</div><div class="month-grid">${cellMarkup}</div></section>`;
  }
  ...
  function renderMultiDayBars(cells){
    ...
    rows.forEach((e,idx)=>{
      ...
      while(cursor<=end){
        ...
        const row=idx%2;
        bars.push(`<div class="multiday-row ${dateKey(cursor)===eventStartDay(e)?'start':'middle'} ${dateKey(segmentEnd)===eventEndDay(e)?'end':''}" style="left:calc((100% / 7) * ${startCol-1} + 4px);width:calc((100% / 7) * ${endCol-startCol} - 8px);top:${week*92 + row*22}px">${dateKey(cursor)===eventStartDay(e)?esc(e.title):''}</div>`);
