(() => {
  ...
  function detailTime(value){const d=new Date(value||'');if(Number.isNaN(d.getTime()))return '日時未設定';return `${d.getMonth()+1}/${d.getDate()}（${'日月火水木金土'[d.getDay()]}） ${String(d.getHours()).padStart(2,'0')}:00`;}
  ...
  function weatherDetailMarkup(detail){
    const hourly=(detail.hourly||[]).map(row=>`<article class="ob36-hourly-row"><time>${esc(detailTime(row.at))}</time><div class="ob36-hourly-condition"><span>${detailWeatherIcon(row.condition)}</span><b>${esc(row.condition)}</b></div><div class="ob36-hourly-primary"><strong>${esc(row.temperature)}°</strong><small>体感 ${esc(row.feelsLike)}°</small></div><div class="ob36-hourly-data"><span><b>${esc(row.rainProbability)}%</b><small>降水確率</small></span><span><b>${esc(row.rainfall)}mm</b><small>降水量</small></span><span><b>${esc(row.windDirection)} ${esc(row.windAverage)}m/s</b><small>平均風速</small></span><span><b>${esc(row.windGust)}m/s</b><small>最大瞬間</small></span><span class="confidence"><b>${esc(row.confidence)}</b><small>信頼度</small></span></div></article>`).join('');
    ...
    const comparisons=(detail.comparisons||[]).map(row=>`<div><b>${esc(row.source)}</b><span>${esc(row.summary)}</span><em>${esc(row.rainProbability)}%・最大${esc(row.windGust)}m/s</em></div>`).join('');
    return `<section class="ob36-detail-summary"><div><small>${esc(detail.place)}　表示サンプル</small><h3>${esc(detail.condition)}</h3><strong>${esc(detail.high)}°／${esc(detail.low)}°</strong></div><div><span><b>${esc(detail.rainPeak)}%</b><small>降水ピーク</small></span><span><b>${esc(detail.windGust)}m/s</b><small>最大瞬間風速</small></span><span><b>${esc(detail.confidence)}</b><small>総合信頼度</small></span></div></section><section class="ob36-hourly-section"><div class="ob36-detail-section-head"><h3>時間帯ごとの予報</h3><span>3時間ごとの表示サンプル</span></div><div class="ob36-hourly-list">${hourly}</div></section><section class="ob36-detail-judgement"><div class="ob36-detail-section-head"><h3>外時間の判断</h3></div>${judgements}</section><section class="ob36-source-compare"><div class="ob36-detail-section-head"><h3>予報サイト比較</h3><span>18日18時</span></div>${comparisons}<p>総合判断：夕方は短時間の雨に注意。サイト間の差があるため信頼度はB＋。</p></section><div class="ob36-detail-meta"><span>優先：${esc(detail.primarySource)}</span><span>比較：${esc((detail.compareSources||[]).join('・')||'未設定')}</span><span>更新：${esc(detail.updatedLabel)}</span></div>`;
  }
  async function openWeatherDetail({mode='plan',place='',start='',end=''}={}){
    ...
    const hourlyIcon='<svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="10" cy="12" r="6"/><path d="M10 8v4l3 2M17 6h4M18 10h3M17 14h4"/></svg>';
    const markup=`<div class="ob36-sheet-backdrop" data-ob36-sheet-backdrop><section class="ob36-sheet ob36-weather-detail-sheet" role="dialog" aria-modal="true" aria-label="時間ごとの詳しい天気"><div class="ob36-sheet-grab"></div><header><div><small>DETAILED WEATHER</small><h2>時間ごとの詳しい天気</h2></div><button type="button" data-ob36-modal-close aria-label="閉じる">×</button></header><div class="ob36-weather-mode"><button type="button" class="${mode==='plan'?'active':''}" data-ob36-weather-mode="plan">予定から見る</button><button type="button" class="${mode==='custom'?'active':''}" data-ob36-weather-mode="custom">場所・日付で見る</button></div>${mode==='plan'?`<div class="ob36-weather-search"><label class="wide"><span>予定</span><select data-ob36-detail-plan>${options}</select></label></div>`:`<div class="ob36-weather-search"><label class="wide"><span>場所</span><input type="text" data-ob36-custom-place value="${esc(customPlace)}" placeholder="キャンプ場・市区町村"></label><label><span>開始日</span><input type="date" data-ob36-custom-start value="${esc(customStart)}"></label><label><span>終了日</span><input type="date" data-ob36-custom-end value="${esc(customEnd)}"></label><button type="button" data-ob36-custom-weather>この条件で見る</button></div>`}<div class="ob36-detail-title-icon">${hourlyIcon}<span>気温・降水・風・信頼度を時間ごとに確認</span></div>${weatherDetailMarkup(detail)}<button class="ob36-sheet-done ob36-sheet-settings" type="button" data-ob36-open-weather-settings>参照サイト設定</button></section></div>`;
  ...
  function snapshot(){return Object.freeze({version:'v166.3-home-v36-r9',requested:requested(),mounted,route:router?.current?.()||null,safe:legacy?.shellSafe?.()??false,cutover:false,previewOnly:true});}
  ...
  function render(reason='refresh'){
    ...
    renderPromise=(async()=>{while(pendingReason){const next=pendingReason;pendingReason=null;await performRender(next);}})().catch(error=>{console.error('[OUTBASE v166.3 HOME v36 r9] shell render failed',error);fallback('render_failed');}).finally(()=>{renderPromise=null;});return renderPromise;
  }
  ...
  async function start(){
    ...
    const detail={status:'ready',version:'v166.3-home-v36-r9',previewOnly:true,cutover:false,route:router.current()};
