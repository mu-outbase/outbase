(() => {
  ...
  async function openWeatherDetail({mode='plan',place='',start='',end='',horizon=null,skipFetch=false,skipRadarFetch=false}={}){
    ...
    const markup=`<div class="ob36-sheet-backdrop" data-ob36-sheet-backdrop><section class="ob36-sheet ob36-weather-detail-sheet" role="dialog" aria-modal="true" aria-label="${title}"><div class="ob36-sheet-grab"></div><header><div><small>WEATHER DETAIL</small><h2>${title}</h2></div><button type="button" data-ob36-modal-close aria-label="閉じる">×</button></header>${modeButtons}${search}${weatherDetailMarkup(detail,radarView)}<button class="ob36-sheet-done ob36-sheet-settings" type="button" data-ob36-open-weather-settings>参考サイト設定</button></section></div>`;
  ...
  function snapshot(){return Object.freeze({version:'v166.3-home-v36-r17',requested:requested(),mounted,route:router?.current?.()||null,safe:legacy?.shellSafe?.()??false,cutover:false,previewOnly:true});}
  ...
  async function start(){
    ...
    const detail={status:'ready',version:'v166.3-home-v36-r17',previewOnly:true,cutover:false,route:router.current()};
