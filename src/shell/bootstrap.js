(() => {
  ...
  function hourlyHighlights(rows){
    ...
    const calm=[...list].sort((a,b)=>{
      const score=row=>hourlyNumber(row.rainProbability)*1.2+hourlyNumber(row.windAverage)*10+Math.max(0,hourlyNumber(row.feelsLike,row.temperature)-28)*5;
      return score(a)-score(b);
    })[0];
    return [
      {label:'雨ピーク',value:`${hourlyTimeLabel(rain)} ${esc(rain?.rainProbability??'—')}%`,tone:weatherTone('rain',rain?.rainProbability)},
      {label:'風ピーク',value:`${hourlyTimeLabel(wind)} ${esc(wind?.windGust??wind?.windAverage??'—')}m/s`,tone:weatherTone('wind',wind?.windGust??wind?.windAverage)},
      {label:'暑さピーク',value:`${hourlyTimeLabel(hot)} 体感${esc(hot?.feelsLike??hot?.temperature??'—')}°`,tone:weatherTone('temperature',hot?.feelsLike??hot?.temperature)},
      {label:'外時間候補',value:`${hourlyTimeLabel(calm)}頃`,tone:'good'}
    ];
  }
  function hourlyCardMarkup(row,index,previousDate=''){
    const dateKey=hourlyDateKey(row);const dateBreak=dateKey&&dateKey!==previousDate?`<span class="ob36-hourly-date-chip">${esc(hourlyDateLabel(row))}</span>`:'';
    return `${dateBreak}<button type="button" class="ob36-hourly-card ${index===0?'active':''}" data-ob36-hourly-card data-hourly-index="${index}" data-hourly-step="${index%3===0?'3':'1'}" aria-pressed="${index===0?'true':'false'}"><time>${esc(hourlyTimeLabel(row))}</time><span class="ob36-hourly-card-icon">${detailWeatherIcon(row.condition)}</span><b class="tone-${weatherTone('temperature',row.temperature)}">${esc(row.temperature)}°</b><small class="tone-${weatherTone('rain',row.rainProbability)}">雨 ${esc(row.rainProbability)}%</small><small class="tone-${weatherTone('wind',row.windAverage)}">風 ${esc(row.windAverage)}m/s</small></button>`;
  }
  function hourlySelectedMarkup(row,index=0){
    if(!row)return '<div class="ob36-hourly-selected-empty">時間を選ぶと詳しい内容を表示します。</div>';
    return `<article class="ob36-hourly-selected" data-ob36-hourly-selected><div class="ob36-hourly-selected-head"><div><small>${esc(hourlyDateLabel(row))}</small><h4>${esc(hourlyTimeLabel(row))}　${esc(row.condition)}</h4></div><span>${detailWeatherIcon(row.condition)}</span></div><div class="ob36-hourly-selected-grid"><div><b class="tone-${weatherTone('temperature',row.temperature)}">${esc(row.temperature)}°</b><small>気温</small></div><div><b>${esc(row.feelsLike)}°</b><small>体感</small></div><div><b class="tone-${weatherTone('rain',row.rainProbability)}">${esc(row.rainProbability)}%</b><small>降水確率</small></div><div><b>${esc(row.rainfall)}mm</b><small>降水量</small></div><div><b class="tone-${weatherTone('wind',row.windAverage)}">${esc(row.windAverage)}m/s</b><small>平均風速</small></div><div><b class="tone-${weatherTone('wind',row.windGust)}">${esc(row.windGust)}m/s</b><small>最大瞬間</small></div><div><b>${esc(row.windDirection)}</b><small>風向</small></div><div><b class="tone-${weatherTone('confidence',row.confidence)}">${esc(row.confidence)}</b><small>信頼度</small></div></div></article>`;
  }
  function hourlyOverviewMarkup(rows){
    ...
    const highlights=hourlyHighlights(list).map(item=>`<div><small>${item.label}</small><b class="tone-${item.tone}">${item.value}</b></div>`).join('');
    return `<section class="ob36-hourly-section"><div class="ob36-detail-section-head"><h3>時間ごとの予報</h3><span>横で流れを見る</span></div><div class="ob36-hourly-highlights">${highlights}</div><div class="ob36-hourly-density" role="group" aria-label="時間間隔"><button type="button" class="active" data-ob36-hourly-density="3">3時間ごと</button><button type="button" data-ob36-hourly-density="1">1時間ごと</button></div><div class="ob36-hourly-rail" data-ob36-hourly-rail>${cards.join('')||'<p class="ob36-intel-empty">この期間の時間別予報はまだありません。</p>'}</div>${hourlySelectedMarkup(list[0],0)}</section>`;
  }
  function weatherDetailMarkup(detail,radarView=null){
  ...
  function bindHourlyRows(host){
    ...
    host.querySelectorAll('[data-ob36-hourly-density]').forEach(button=>button.addEventListener('click',()=>{const step=button.dataset.ob36HourlyDensity||'3';host.querySelectorAll('[data-ob36-hourly-density]').forEach(item=>item.classList.toggle('active',item===button));rows.forEach(item=>{item.hidden=step==='3'&&item.dataset.hourlyStep!=='3';});if(rail)rail.scrollLeft=0;const first=Array.from(rows).find(item=>!item.hidden);if(first)activate(first);}));
  }
  ...
  async function openWeatherDetail({mode='plan',place='',start='',end='',horizon=null,skipFetch=false,skipRadarFetch=false,planScrollLeft=null}={}){
    ...
    const selectedHorizon=radarHorizon(horizon);const radarStartAt=mode==='plan'?(selected?.startAt||targetStart):mode==='custom'?`${targetStart}T00:00:00`:new Date().toISOString();
    if(!skipRadarFetch&&navigator.onLine){try{await radarService()?.refresh?.();}catch(_error){}}
    ...
    host.__ob36HourlyRows=Array.isArray(detail.hourly)?detail.hourly:[];bindHourlyRows(host);bindExternalWeatherLinks(host,{detail,mode,place:targetPlace,start:targetStart,end:targetEnd});bindRainRadar(host,{detail,mode,place:targetPlace,start:targetStart,end:targetEnd,horizon:selectedHorizon,radarStartAt});
  }
  ...
  function snapshot(){return Object.freeze({version:'v166.3-home-v36-r29',requested:requested(),mounted,route:router?.current?.()||null,safe:legacy?.shellSafe?.()??false,cutover:false,previewOnly:true});}
  ...
  async function start(){
    ...
    const detail={status:'ready',version:'v166.3-home-v36-r29',previewOnly:true,cutover:false,route:router.current()};
