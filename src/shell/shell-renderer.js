(() => {
  ...
  const icons={
    ...
    weatherNow:'<svg class="ob36-weather-glyph" viewBox="0 0 48 48" aria-hidden="true"><circle class="sun-fill" cx="17" cy="17" r="7"/><circle class="sun-line" cx="17" cy="17" r="7"/><path class="sun-rays" d="M17 5v4M17 25v4M5 17h4M25 17h4M8.5 8.5l2.8 2.8M22.7 22.7l2.8 2.8M25.5 8.5l-2.8 2.8"/><path class="cloud-fill" d="M13 37h24a6 6 0 0 0 .4-12 9 9 0 0 0-17-2.7A7.2 7.2 0 0 0 13 37Z"/><path class="cloud-line" d="M13 37h24a6 6 0 0 0 .4-12 9 9 0 0 0-17-2.7A7.2 7.2 0 0 0 13 37Z"/></svg>',
    weatherInsight:'<svg class="ob36-intel-glyph" viewBox="0 0 48 48" aria-hidden="true"><path class="frame" d="M24 5 39 13.5v17L24 39 9 30.5v-17Z"/><circle class="sun" cx="17" cy="17" r="4.5"/><path class="cloud" d="M15 31h19a4.7 4.7 0 0 0 .3-9.4 7 7 0 0 0-13.2-2A5.7 5.7 0 0 0 15 31Z"/><path class="trend" d="m16 34 5-5 4 3 7-8M29 24h3v3"/></svg>',
  ...
  function ob36Today(model){
    ...
    const summary=`<div class="ob36-today-summary"><div><small>${esc(model.home.todayLabel)}</small><h1>${esc(model.home.todaySummary)}</h1></div><span class="ob36-scope"><button type="button" class="${active==='home'?'active':''}" data-ob36-weather-scope="home">自宅</button><button type="button" class="${active==='current'?'active':''}" data-ob36-weather-scope="current">現在地</button></span></div>`;
    if(weather.status!=='ready')return `<section class="ob36-weather-card ob36-card is-unavailable">${summary}<div class="ob36-weather-unavailable"><span class="ob36-weather-symbol">${icons.weatherNow}</span><div class="ob36-weather-unavailable-copy"><div class="ob36-weather-head"><span>${esc(weather.whenLabel||'今日')}</span><em>${esc(weather.locationLabel||'場所未設定')}</em></div><div class="ob36-weather-state"><b>${esc(weather.condition||'天気情報未接続')}</b><span><i aria-hidden="true"></i>接続待ち</span></div><p>気温・降水確率・風速は天気接続後に表示します。</p></div></div></section>`;
    const temp=weather.temperature===null||weather.temperature===undefined?'—':Number(weather.temperature).toFixed(1).replace(/\.0$/,'');
    const high=weather.high===null||weather.high===undefined?'—':weather.high;const low=weather.low===null||weather.low===undefined?'—':weather.low;
    return `<section class="ob36-weather-card ob36-card is-ready">${summary}<div class="ob36-weather-lower"><span class="ob36-weather-symbol">${icons.weatherNow}</span><div class="ob36-weather-content"><div class="ob36-weather-head"><span>${esc(weather.whenLabel||'今日')}</span><em>${esc(weather.locationLabel||'自宅')}</em></div><div class="ob36-weather-body"><div class="ob36-current"><strong>${esc(temp)}°</strong><b>${esc(weather.condition||'天気情報未接続')}</b><small>${esc(high)}°／${esc(low)}°</small></div><div class="ob36-metric"><b>降水確率</b><span>${ob36Value(weather.rain,'%')}</span></div><div class="ob36-metric"><b>風速</b><span>${ob36Value(weather.wind,' m/s')}</span></div></div></div></div></section>`;
  }
  ...
  function ob36Intel(model){
    ...
    if(!selected)return `<section class="ob36-intel ob36-card is-empty"><div class="ob36-intel-title"><h2>天気インテリジェンス</h2>${icons.info}</div><p class="ob36-intel-empty">対象になる予定がありません。予定を登録すると、ここで天気判断を確認できます。</p></section>`;
    const header=`<div class="ob36-intel-title"><h2>天気インテリジェンス</h2>${icons.info}<div><select data-ob36-weather-plan aria-label="対象予定">${options}</select></div></div><p class="ob36-intel-place">${esc(intel.place||selected.place||'場所未設定')}　サイト周辺</p><div class="ob36-intel-range"><strong>${esc(dateRange(selected))}</strong><span>${esc(intel.durationLabel||'')}</span></div>`;
    const checks=Array.isArray(intel.checks)?intel.checks:[];
    const checkHtml=checks.map(check=>`<div class="ob36-judgement-item tone-${esc(check.id)}"><span class="ob36-judgement-icon ${check.icon==='paw'?'paw':''}">${icons[check.icon]||icons.info}</span><span><b>${esc(check.label)}</b><small>${esc(check.detail)}</small></span><em>未判定</em></div>`).join('');
    if(intel.status!=='ready')return `<section class="ob36-intel ob36-card is-unavailable">${header}<div class="ob36-intel-unavailable"><span class="ob36-intel-weather-art">${icons.weatherInsight}</span><div><small>キャンプ判断</small><h3>接続後に自動判定</h3><p>${esc(intel.message||'雨・風・気温・設営・撤収・ペットへの影響を判定します。')}</p></div><span class="ob36-intel-status"><i aria-hidden="true"></i>接続待ち</span></div><div class="ob36-judgement-grid">${checkHtml}</div><div class="ob36-intel-meta"><span>予報更新：${esc(intel.updatedLabel||'未取得')}</span><span>信頼度：${esc(intel.confidenceLabel||'未判定')}</span></div></section>`;
    return `<section class="ob36-intel ob36-card is-ready">${header}<div class="ob36-intel-summary"><span>${icons.weatherInsight}</span><div><small>期間中の天気傾向</small><h3>${esc(intel.condition||'予報データ未接続')}</h3><b>${ob36Value(intel.high,'°')}／${ob36Value(intel.low,'°')}</b><div class="ob36-intel-metrics"><div><small>降水ピーク</small><b>${ob36Value(intel.rainPeak,'%')}</b></div><div><small>最大風速</small><b>${ob36Value(intel.windMax,' m/s')}</b></div><div><small>信頼度</small><b>${ob36Value(intel.confidence)}</b></div></div></div></div><div class="ob36-intel-message">${esc(intel.message||'天気判断を確認してください。')}</div><button type="button" class="ob36-intel-detail">ピンポイント天気詳細を見る　›</button></section>`;
  }
