(() => {
  ...
  const icons={
    ...
    weather:'<svg viewBox="0 0 80 64" aria-hidden="true"><circle cx="27" cy="23" r="16" fill="#FFF2B8" opacity=".45"/><circle cx="27" cy="23" r="11.5" fill="#FFD45B" stroke="#D99B1F" stroke-width="1.15"/><path d="M27 3v7M27 36v7M7 23h7M40 23h7M12.8 8.8l5 5M36.2 32.2l5 5M41.2 8.8l-5 5" stroke="#D99B1F" stroke-width="1.55" stroke-linecap="round"/><path d="M17 53h41a9 9 0 0 0 0-18 13.1 13.1 0 0 0-24.9-2.8A10.7 10.7 0 0 0 17 53Z" fill="#FAFCFC" stroke="#B3BEC2" stroke-width="1"/><path d="M11 57h36M50 59h16" stroke="#8FB4C2" stroke-width="1.25" stroke-linecap="round" opacity=".72"/><path d="M19 48c3.6-6.1 9.4-8.1 15.2-5.4" stroke="#fff" stroke-width="2" stroke-linecap="round" opacity=".9"/></svg>',
  ...
  function ob36Intel(model){
    ...
    if(intel.status!=='ready')return `<section class="ob36-intel ob36-card is-unavailable">${header}<div class="ob36-intel-unavailable"><span class="ob36-intel-weather-art">${icons.weather}<i>接続待ち</i></span><div><small>期間中の天気傾向</small><h3>${esc(intel.condition||'予報データ未接続')}</h3><p>${esc(intel.message||'実予報は天気接続後に表示します。')}</p></div></div></section>`;
    return `<section class="ob36-intel ob36-card is-ready">${header}<div class="ob36-intel-summary"><span>${icons.weather}</span><div><small>期間中の天気傾向</small><h3>${esc(intel.condition||'予報データ未接続')}</h3><b>${ob36Value(intel.high,'°')}／${ob36Value(intel.low,'°')}</b><div class="ob36-intel-metrics"><div><small>降水ピーク</small><b>${ob36Value(intel.rainPeak,'%')}</b></div><div><small>最大風速</small><b>${ob36Value(intel.windMax,' m/s')}</b></div><div><small>信頼度</small><b>${ob36Value(intel.confidence)}</b></div></div></div></div><div class="ob36-intel-message">${esc(intel.message||'天気判断を確認してください。')}</div><button type="button" class="ob36-intel-detail">ピンポイント天気詳細を見る　›</button></section>`;
  }
