(() => {
  ...
  function nextBand(item){
    ...
    return `<article class="ob6-next-band ob7-next-band ob8-next-band type-${esc(typeKey(item))}" data-activity-id="${esc(item.id)}" style="view-transition-name:ob-activity-${token(item.id)}">${activityVisual(item,{className:'ob7-next-visual ob8-next-visual',label:false})}<span class="ob8-next-visual-meta"><b>${esc(dateLabel(item.startAt,{weekday:false}))}</b><small>${esc(item.place||item.typeLabel||'活動')}</small></span><div class="ob7-next-content"><div class="ob6-next-top"><span>次の${esc(item.typeLabel||'活動')}</span><b>${esc(item.relativeDay)}</b></div><h3>${esc(item.title)}</h3>${meta(item)}<div class="ob6-prep"><b>準備 ${prep.completed}/${prep.total}</b><div class="ob4-progress-track"><i style="width:${Math.max(0,Math.min(100,Number(prep.progress)||0))}%"></i></div><span>${Number(prep.progress)||0}%</span></div><div class="ob6-next-actions">${shellLink('activity',{activityId:item.id},'活動を見る','ob6-secondary light')}<a class="ob6-primary brass" href="${esc(item.preparationUrl)}">準備する</a></div></div></article>`;
  }
  ...
  function search(){return `<section class="ob6-page-hero ob7-page-hero"><h1>探す</h1><p>場所・写真・予定の中身を見ながら、次の入口を選べます。</p></section><section class="ob7-search-grid ob8-search-grid">${searchCard({title:'キャンプ場や候補',sub:'地図から場所と候補を探す',action:'search',kind:'legacy',tone:'camp',icon:'camp'})}${searchCard({title:'過去の記録',sub:'写真と活動ストーリーを見る',action:'vault',tone:'memory',icon:'photo'})}${searchCard({title:'これからの予定',sub:'カレンダーと家族予定を確認',action:'calendar',tone:'calendar',icon:'calendar'})}</section>`;}
  ...
  function vault(model){
    ...
    const feature=latest?`<a class="ob7-vault-feature ob8-vault-feature" href="${esc(globalThis.OUTBASE_ROUTER.shellUrl('activity',{activityId:latest.id}))}" data-ob5-nav data-ob5-route="activity" data-ob5-activity-id="${esc(latest.id)}">${activityVisual(latest,{className:'ob7-vault-visual ob8-vault-feature-visual',label:false})}<span class="ob8-vault-feature-copy"><small>最近の活動</small><b>${esc(latest.title)}</b><em>${esc(dateLabel(latest.startAt))}・記録 ${Number(latest.recordCount||0)}件${latest.mediaCount?`・写真/動画 ${Number(latest.mediaCount)}件`:''}</em></span><span class="ob8-vault-feature-arrow">${icons.arrow}</span></a>`:'';
    const gallery=activities.length?`<section class="ob6-section ob8-vault-gallery-section"><div class="ob6-section-title"><h2>写真と活動</h2><button data-ob3-legacy="vault">持ち物を見る</button></div><div class="ob8-vault-gallery">${activities.slice(0,4).map(vaultTile).join('')}</div></section>`:'';
    return `<section class="ob6-page-hero ob7-page-hero"><h1>保管庫</h1><p>写真を入口に、活動ごとの記録と思い出をたどれます。</p></section>${feature}<section class="ob6-metric-strip ob7-metric-strip"><div><small>活動</small><b>${model.vaultSummary.activityCount}</b></div><div><small>記録</small><b>${model.vaultSummary.recordCount}</b></div><div><small>持ち物</small><b>${model.vaultSummary.assetCount}</b></div></section>${gallery}<section class="ob6-section"><div class="ob6-section-title"><h2>すべての活動</h2><span>${activities.length}件</span></div><div class="ob6-story-list ob7-story-list">${rows}</div></section>`;
  }
  ...
  function centralSheet(){return `<div class="ob3-backdrop" data-ob3-backdrop><section class="ob3-sheet ob7-sheet ob8-sheet" role="dialog" aria-modal="true" aria-label="追加する"><div class="ob3-sheet-handle"></div><div class="ob3-sheet-head"><div><h2>何をする？</h2></div><button data-ob3-close aria-label="閉じる">×</button></div><div class="ob3-sheet-actions ob8-sheet-actions"><button class="tone-field ob8-sheet-primary" data-ob3-action="start"><span>${icons.play}</span><div><small>現地で記録する</small><b>活動を始める</b><em>GPS・写真・音声をひとつの活動へ</em></div><strong>始める</strong></button><div class="ob8-sheet-secondary"><button class="tone-memory" data-ob3-action="memo"><span>${icons.memo}</span><div><b>メモを残す</b><small>気づきをすぐ記録</small></div>${icons.arrow}</button><button class="tone-calendar" data-ob3-action="plan-add"><span>${icons.calendar}</span><div><b>予定を追加</b><small>カレンダーへ追加</small></div>${icons.arrow}</button></div></div></section></div>`;}
  ...
  const api=Object.freeze({mount,showCentral,hideCentral,updateNetwork,updateNav,hydrateMedia,activityVisual,sceneSvg,homeHtml:home,searchHtml:search,vaultHtml:vault,activityHtml:activityDetail,calendarHtml:calendar,storyRow,nextBand});
