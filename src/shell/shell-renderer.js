(() => {
  ...
  function ob36Intel(model){
    ...
    const alerts=(Array.isArray(intel.alerts)?intel.alerts:[]).slice(0,3).map(alert=>`<div class="ob36-notice-row"><time>${esc(alert.time)}</time><span><b>${esc(alert.title)}</b><small>${esc(alert.detail)}</small></span></div>`).join('');
    return `<section class="ob36-intel ob36-card is-ready ${intel.sample?'is-sample':''}"><div class="ob36-intel-title"><h2>天気の見立て</h2><div><select data-ob36-weather-plan aria-label="対象予定">${options}</select></div></div><p class="ob36-intel-place">${esc(intel.place||selected.place||'場所未設定')}　<span>表示サンプル</span></p><div class="ob36-intel-range"><strong>${esc(dateTimeRange(selected))}</strong><span>${esc(intel.durationLabel||'')}</span></div><div class="ob36-intel-weather-summary"><span class="ob36-intel-weather-icon">${icons.insight}</span><div class="ob36-intel-weather-copy"><small>期間中の天気傾向</small><div class="ob36-intel-condition"><h3>${esc(intel.condition)}</h3><strong>${esc(intel.high)}°／${esc(intel.low)}°</strong></div><div class="ob36-intel-weather-line"><div><b>降水ピーク</b><span>${esc(intel.rainPeak)}%</span></div><div><b>最大風速</b><span>${esc(intel.windMax)} m/s</span></div><div><b>信頼度</b><span>${esc(intel.confidenceLabel||intel.confidence)}</span></div></div></div></div><div class="ob36-notice-panel"><h3>今回の注意</h3><p>${esc(intel.message)}</p><div class="ob36-notice-list">${alerts}</div></div><button type="button" class="ob36-intel-detail-button" data-ob36-weather-detail><span class="ob36-intel-detail-icon">${icons.hourly}</span><span><b>時間ごとの詳しい天気</b><small>気温・雨・風・予報信頼度を確認</small></span>${icons.arrow}</button></section>`;
  }
