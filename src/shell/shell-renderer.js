(() => {
  ...
  const icons={
    ...
    weather:'<svg viewBox="0 0 64 56" aria-hidden="true"><circle cx="22" cy="21" r="10.8" fill="#FFD45B" stroke="#D99B1F" stroke-width="1.05"/><path d="M22 3v6M22 33v6M4 21h6M34 21h6M9 8l4.1 4.1M31 30l4.1 4.1M35 8l-4.1 4.1" stroke="#D99B1F" stroke-width="1.45" stroke-linecap="round"/><path d="M16.5 47H48a7.4 7.4 0 0 0 0-14.8 11.1 11.1 0 0 0-20.8-2.3A8.9 8.9 0 0 0 16.5 47Z" fill="#F5F8F8" stroke="#B8C2C6" stroke-width=".85"/></svg>',
  ...
  function ob36Today(model){
    ...
    return `<section class="ob36-weather-card ob36-card"><div class="ob36-today-summary"><div><small>${esc(model.home.todayLabel)}</small><h1>${esc(model.home.todaySummary)}</h1></div><span class="ob36-scope"><button type="button" class="${active==='home'?'active':''}" data-ob36-weather-scope="home">自宅</button><button type="button" class="${active==='current'?'active':''}" data-ob36-weather-scope="current">現在地</button></span></div><div class="ob36-weather-lower"><span class="ob36-weather-symbol">${icons.weather}</span><div class="ob36-weather-content"><div class="ob36-weather-head"><span>${esc(weather.whenLabel||'今日')}</span><em>${esc(weather.locationLabel||'自宅')}</em></div><div class="ob36-weather-body"><div class="ob36-current"><strong>${esc(temp)}°</strong><b>${esc(weather.condition||'天気情報未接続')}</b><small>${esc(high)}°／${esc(low)}°</small></div><div class="ob36-metric"><b>降水確率</b><span>${ob36Value(weather.rain,'%')}</span></div><div class="ob36-metric"><b>風速</b><span>${ob36Value(weather.wind,' m/s')}</span></div></div></div></div></section>`;
  }
  function ob36PlanCard(item,selected){
    const prep=item.preparation||{completed:0,total:0,progress:0};
    const href=globalThis.OUTBASE_ROUTER.shellUrl('activity',{activityId:item.id});
    return `<a class="ob36-plan ${selected?'selected':''}" href="${esc(href)}" data-ob5-nav data-ob5-route="activity" data-ob5-activity-id="${esc(item.id)}"><span class="ob36-plan-tag">${esc(item.typeLabel||'活動')}</span>${activityVisual(item,{className:'ob36-plan-photo',label:false})}<h3>${esc(item.title)}</h3><p>${esc(dateRange(item))}</p><div class="ob36-plan-temp"><span>—°／—°</span><em>${prep.total?`準備 ${prep.completed}/${prep.total}`:'準備未設定'}</em></div></a>`;
  }
  function ob36Plans(model){
    const rows=model.home.next||[];
    const body=rows.length?rows.slice(0,5).map(item=>ob36PlanCard(item,item.id===model.home.selectedPlanId)).join(''):`<div class="ob36-empty"><b>今後の予定はまだありません。</b><button type="button" data-ob3-action="plan-add">予定を追加</button></div>`;
    return `<section class="ob36-plans ob36-card"><div class="ob36-section-head"><h2>今後の予定</h2><button type="button" data-ob3-action="calendar">すべて見る　›</button></div><div class="ob36-plan-strip">${body}</div></section>`;
  }
  function ob36Intel(model){
    ...
    if(!selected)return `<section class="ob36-intel ob36-card"><div class="ob36-intel-title"><h2>天気インテリジェンス</h2>${icons.info}</div><p class="ob36-intel-empty">対象になる予定がありません。予定を登録すると、ここで天気判断を確認できます。</p></section>`;
    return `<section class="ob36-intel ob36-card"><div class="ob36-intel-title"><h2>天気インテリジェンス</h2>${icons.info}<div><select data-ob36-weather-plan aria-label="対象予定">${options}</select></div></div><p class="ob36-intel-place">${esc(intel.place||selected.place||'場所未設定')}　サイト周辺</p><div class="ob36-intel-range"><strong>${esc(dateRange(selected))}</strong><span>${esc(intel.durationLabel||'')}</span></div><div class="ob36-intel-summary"><span>${icons.weather}</span><div><small>期間中の天気傾向</small><h3>${esc(intel.condition||'予報データ未接続')}</h3><b>—°／—°</b><div class="ob36-intel-metrics"><div><small>降水ピーク</small><b>—%</b></div><div><small>最大風速</small><b>— m/s</b></div><div><small>信頼度</small><b>—</b></div></div></div></div><div class="ob36-intel-message">${esc(intel.message||'実予報は天気接続後に表示します。')}</div><button type="button" class="ob36-intel-detail" disabled aria-disabled="true">ピンポイント天気詳細は接続準備中</button></section>`;
  }
  function home(model){return `${ob36Topbar()}${ob36Today(model)}${ob36Plans(model)}${ob36Intel(model)}${quick(model)}`;}
