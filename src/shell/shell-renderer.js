(() => {
  ...
  function ob36Today(model){
    ...
    if(!hasData)return `<section class="ob36-weather-card ob36-card is-unavailable">${title}<div class="ob36-weather-unavailable"><span class="ob36-weather-symbol">${icons.weatherNow}</span><div class="ob36-weather-unavailable-copy"><div class="ob36-weather-state"><b>${esc(weather.condition||'天気情報未接続')}</b><span><i aria-hidden="true"></i>接続待ち</span></div><p>気温・降水確率・風速は天気接続後に表示します。</p></div></div>${updateMarkup(weather.update)}</section>`;
  ...
  function ob36Intel(model){
    ...
    return `<section class="ob36-intel ob36-card is-ready ${intel.sample?'is-sample':''}"><div class="ob36-intel-title"><h2>予定の天気見立て</h2><div><select data-ob36-weather-plan aria-label="対象予定">${options}</select></div></div><p class="ob36-intel-place">${esc(intel.place||selected.place||'場所未設定')}　<span>表示サンプル</span></p><div class="ob36-intel-range"><strong>${esc(dateTimeRange(selected))}</strong><span>${esc(intel.durationLabel||'')}</span></div><div class="ob36-intel-weather-summary"><span class="ob36-intel-weather-icon">${icons.insight}</span><div class="ob36-intel-weather-copy"><small>期間中の天気傾向</small><div class="ob36-intel-condition"><h3>${esc(intel.condition)}</h3><strong class="${metricClass('temperature',intel.high)}">${esc(intel.high)}°／${esc(intel.low)}°</strong></div><div class="ob36-intel-metric-row"><div><b class="${metricClass('rain',intel.rainPeak)}">${esc(intel.rainPeak)}%</b><small>降水ピーク</small></div><div><b class="${metricClass('wind',intel.windMax)}">${esc(intel.windMax)}m/s</b><small>最大風速</small></div><div><b class="${metricClass('confidence',intel.confidenceLabel||intel.confidence)}">${esc(intel.confidenceLabel||intel.confidence)}</b><small>信頼度</small></div></div></div></div><div class="ob36-notice-panel"><div class="ob36-notice-heading"><h3>今回の注意</h3><p>${esc(intel.message)}</p></div><div class="ob36-notice-list">${alerts}</div></div><button type="button" class="ob36-intel-detail-button" data-ob36-weather-detail><span class="ob36-intel-detail-icon">${icons.hourly}</span><span><b>時間ごとの天気</b><small>一覧はシンプル、押すと詳しく表示</small></span>${icons.arrow}</button>${updateMarkup(intel.update,true)}</section>`;
  }
