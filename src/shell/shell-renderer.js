(() => {
  ...
  function ob36Intel(model){
    ...
    if(intel.status!=='ready')return `<section class="ob36-intel ob36-card is-unavailable">${header}<div class="ob36-intel-unavailable"><span>${icons.weather}</span><div><small>期間中の天気傾向</small><h3>${esc(intel.condition||'予報データ未接続')}</h3><p>${esc(intel.message||'実予報は天気接続後に表示します。')}</p></div></div></section>`;
