(() => {
  ...
  function nextBand(item){
    ...
    return `<article class="ob6-next-band ob7-next-band ob8-next-band ob9-next-hero type-${esc(displayTypeKey(item))}" data-activity-id="${esc(item.id)}" style="view-transition-name:ob-activity-${token(item.id)}">${activityVisual(item,{className:'ob7-next-visual ob8-next-visual',label:false})}<span class="ob8-next-visual-meta"><b>${esc(dateLabel(item.startAt,{weekday:false}))}</b><small>${esc(item.place||item.typeLabel||'活動')}</small></span><div class="ob7-next-content"><div class="ob6-next-top"><span>次の${esc(item.typeLabel||'活動')}</span><b>${esc(item.relativeDay)}</b></div><h3>${esc(item.title)}</h3>${meta(item)}<div class="ob6-prep"><b>準備 ${prep.completed}/${prep.total}</b><div class="ob4-progress-track"><i style="width:${Math.max(0,Math.min(100,Number(prep.progress)||0))}%"></i></div><span>${Number(prep.progress)||0}%</span></div><div class="ob6-next-actions">${shellLink('activity',{activityId:item.id},'活動を見る','ob6-secondary light')}<a class="ob6-primary brass" href="${esc(item.preparationUrl)}">準備する</a></div></div></article>`;
  }
  ...
  function quick(model){
    const items=model.home.quick||[];const first=items[0];const rest=items.slice(1);
    return `<div class="ob9-quick">${first?`<button class="ob9-quick-primary" data-ob3-action="${esc(first.action)}"><span>${icons[first.icon]||icons.play}</span><div><small>NOW</small><b>${esc(first.label)}</b><em>${esc(first.hint)}</em></div>${icons.arrow}</button>`:''}<div class="ob9-quick-secondary">${rest.map(item=>`<button data-ob3-action="${esc(item.action)}"><span>${icons[item.icon]||icons.arrow}</span><div><b>${esc(item.label)}</b><small>${esc(item.hint)}</small></div>${icons.arrow}</button>`).join('')}</div></div>`;
  }
  function recentRailItem(item){
    return `<a class="ob9-recent-item" href="${esc(globalThis.OUTBASE_ROUTER.shellUrl('activity',{activityId:item.id}))}" data-ob5-nav data-ob5-route="activity" data-ob5-activity-id="${esc(item.id)}">${activityVisual(item,{className:'ob9-recent-visual',label:false})}<span><small>${esc(dateLabel(item.startAt,{weekday:false}))} · ${esc(item.typeLabel||'活動')}</small><b>${esc(item.title)}</b><em>${Number(item.recordCount||0)}件の記録${item.mediaCount?` · 写真/動画 ${Number(item.mediaCount)}件`:''}</em></span></a>`;
  }
  function home(model){
    const current=model.home.current||[],next=model.home.next||[],recent=model.home.recent||[];
    const nowHtml=current.length?current.slice(0,1).map(currentLine).join(''):emptyNow();
    const nextHtml=next.length?nextBand(next[0]):'<p class="ob3-empty">近い予定はまだありません。</p>';
    const following=next.slice(1,3).length?`<div class="ob9-following"><small>その次</small>${next.slice(1,3).map(storyRow).join('')}</div>`:'';
    const recentHtml=recent.length?recent.slice(0,6).map(recentRailItem).join(''):'<p class="ob3-empty">最近の思い出はまだありません。</p>';
    return `<section class="ob6-home-hero ob7-home-hero ob9-home-hero"><div><small>OUTBASE / NORTH</small><h1>${esc(model.home.todayLabel)}</h1><p>予定、現地記録、思い出をひとつの流れで。</p></div><button data-ob5-nav data-ob5-route="calendar" aria-label="カレンダーを開く">${icons.calendar}</button></section>${family(model)}<section class="ob9-now"><div class="ob9-kicker"><span>NOW</span><h2>今</h2></div>${nowHtml}</section><section class="ob9-next-section"><div class="ob9-kicker"><span>NEXT</span><h2>次</h2><button data-ob5-nav data-ob5-route="calendar">カレンダー ${icons.arrow}</button></div>${nextHtml}${following}</section><section class="ob9-quick-section"><div class="ob9-kicker"><span>TOOLS</span><h2>すぐ使う</h2></div>${quick(model)}</section><section class="ob9-recent-section"><div class="ob9-kicker"><span>STORIES</span><h2>最近</h2><button data-ob3-route="vault">すべて見る ${icons.arrow}</button></div><div class="ob9-recent-rail">${recentHtml}</div></section>`;
  }
