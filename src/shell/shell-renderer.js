(() => {
  ...
  function dateTimeRange(item){const start=new Date(item?.startAt||'');const end=new Date(item?.endAt||item?.startAt||'');if(Number.isNaN(start.getTime()))return '日時未設定';const date=d=>`${d.getMonth()+1}/${d.getDate()}（${'日月火水木金土'[d.getDay()]}）`;const time=d=>`${d.getHours()}:${String(d.getMinutes()).padStart(2,'0')}`;if(Number.isNaN(end.getTime()))return `${date(start)} ${time(start)}`;const same=start.getFullYear()===end.getFullYear()&&start.getMonth()===end.getMonth()&&start.getDate()===end.getDate();return same?`${date(start)} ${time(start)}〜${time(end)}`:`${date(start)} ${time(start)}〜${date(end)} ${time(end)}`;}
  ...
  function providerLabel(value){return String(value||'Open-Meteo').replace(/\s*Best Match\s*/gi,'').trim()||'Open-Meteo';}
