(() => {
  ...
  function latestMarkup(item){
    if(!item)return `<section class="ob13-vault-latest empty"><span>${icons.activity}</span><div><small>LATEST</small><b>活動ストーリーはまだありません</b><p>活動を終了して整理すると、ここに残ります。</p></div></section>`;
  ...
  function panelMarkup(id,title,count,body,action=''){return `<section class="ob13-vault-panel" data-vault-panel="${esc(id)}"><div class="ob13-vault-section-head"><div><small>${esc(id.toUpperCase())}</small><h2>${esc(title)}</h2></div><span>${Number(count||0)}件</span></div>${action}<div class="ob13-vault-list">${body||`<div class="ob13-vault-empty"><b>まだありません</b><p>追加・記録した内容がここへ整理されます。</p></div>`}</div></section>`;}
