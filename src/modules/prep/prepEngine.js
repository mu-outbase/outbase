import { prepBase } from '../../domain/schema.js?v=core06-05-all-pages-visual-unify-20260704';
...
export function buildPracticalPrep(project = {}, context = {}) {
  ...
  if (context.menuMemo) { prependItems(prep.shopping, menuShoppingItems(context.menuMemo)); splitMemo(context.menuMemo).slice(0, 4).forEach((line) => prep.shopping.unshift(`献立：${line}`)); }
  if (context.routeMemo) { prep.reflection.unshift(`行き方：${context.routeMemo}`); prep.packing.unshift('出発時間 / 経由地 / 休憩ポイントを確認'); }
  if (context.setupMemo) { prep.reflection.unshift(`設営・撤収：${context.setupMemo}`); prep.packing.unshift('設営・撤収タイマーで時間を残す'); }
  if (context.campgroundSearchMemo) prep.reflection.unshift(`キャンプ場探し条件：${context.campgroundSearchMemo}`);
  ...
  if (context.kotaGoing !== 'no') prep.kota.unshift('ドッグカート / フード / 足拭き / 予備リードを玄関側にまとめる');
  else prep.kota = ['コタ同行なし。ペット用品は不要または留守番側で確認'];
  if (context.pastReflection) { prependItems(prep.reflection, reflectionActionItems(context.pastReflection)); splitMemo(context.pastReflection).slice(0, 5).forEach((line) => prep.reflection.unshift(`前回反省：${line}`)); }
  if (context.gearMemo) { prependItems(prep.packing, gearPackingItems(context.gearMemo)); splitMemo(context.gearMemo).slice(0, 5).forEach((line) => prep.packing.unshift(`ギア確認：${line}`)); }
...
function reflectionActionItems(value) { const text = compactJapaneseSpacing(value); const items = []; if (/雨|撤収|濡/.test(text)) items.push('雨撤収用に濡れ物バッグと収納順を先に決める'); if (/タオル|不足/.test(text)) items.push('人用・コタ用の予備タオルを追加'); if (/料理|多すぎ|余り/.test(text)) items.push('食材量を2人×1泊基準に絞る'); if (/忘れ/.test(text)) items.push('前回の忘れ物を確認'); return items; }
function prependItems(target, items = []) { [...items].reverse().forEach((item) => { if (item) target.unshift(item); }); }
...
function findTimeByLabel(text = '', labels = []) { for (const label of labels) { const regex = new RegExp(`${label}[^0-9]{0,12}(\\d{1,2}:\\d{2})`, 'i'); const hit = text.match(regex); if (hit) return hit[1]; } return '未確定'; }
