...
export function renderHome() {
  ...
  const projectTitle = state.nextProject?.title || '次のキャンプ未作成';
  app().innerHTML = [
    card(`<div class="title">OUTBASEは、次のキャンプを楽にする。</div><p class="muted">MVPα1は凍結。本開発は、予約情報→準備候補→当日記録→次回改善の流れで作る。</p>`, 'hero'),
    card(`<h2>次のキャンプ</h2><p><strong>${projectTitle}</strong></p><p class="muted">手入力カードではなく、予約スクショ/PDF/メール/カレンダー等から候補化して作る。</p><button class="btn primary" id="goPrep">準備エンジンを開く</button>`),
