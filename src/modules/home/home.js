...
export function renderHome() {
  ...
  app().innerHTML = [
    card(`<div class="title">OUTBASEは、準備と記録を次に活かす。</div><p class="muted">Core03.1は、キャンプ当日・コタ散歩・日常メモを3秒で残し、履歴カード・詳細画面・次回改善へつなぐ。</p>`, 'hero'),
    ...
    card(`<h2>次のキャンプ</h2><p><strong>${escapeHtml(projectTitle)}</strong></p><p class="muted">${project ? `準備候補 ${prepCount}件。Core03.1で当日記録・散歩・日常メモ・詳細画面へ接続。` : '予約スクショ/PDF/メール/カレンダー等から候補化して作る。'}</p><div class="grid"><button class="btn primary" id="goPrep">準備エンジン</button><button class="btn" id="goRecord">記録を開く</button></div>`),
    card(`<h2>最近の記録</h2>${recent.length ? `<ul class="outbase-list">${recent.map((item) => `<li><strong>${escapeHtml(sessionTypeLabel(item.type))}</strong> ${escapeHtml(formatDateTime(item.startedAt))} / 記録 ${(item.records || []).length}件</li>`).join('')}</ul>` : '<p class="muted">まだ記録はありません。Core03.1で散歩・キャンプ当日・日常メモを残します。</p>'}`),
    card(`<h2>今の到達点</h2><div class="timeline"><div><strong>Core01</strong><span>予約→候補→次キャンプカードまで確認済み</span></div><div><strong>Core02</strong><span>準備候補の実用化を通過</span></div><div><strong>Core03</strong><span>記録の詳細表示と改善候補までつなぐ</span></div></div>`),
