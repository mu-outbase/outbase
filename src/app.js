...
(() => {
  ...
  function restoreScroll(y){
    ...
    const run=()=>{try{window.scrollTo(0,y);document.documentElement.scrollTop=y;document.body.scrollTop=y}catch(e){}};
    const raf=window.requestAnimationFrame||((fn)=>setTimeout(fn,0));
    raf(run);setTimeout(run,60);setTimeout(run,180);
  }
  if(!window.__outbaseScrollGuard){
    window.__outbaseScrollGuard=true;
    document.addEventListener('pointerdown',e=>{if(e.target.closest('button,[data-act],[data-prep],[data-route],[data-field-mode],[data-memory],[data-discover]'))rememberScroll()},true);
    document.addEventListener('click',e=>{if(e.target.closest('button,[data-act],[data-prep],[data-route],[data-field-mode],[data-memory],[data-discover]'))rememberScroll()},true);
  }
  ...
  function migrateState(s){
    ...
    s.routePlan.stops={gas:true,conv:true,sa:true,kota1:true,afterConv:true,super:true,michi:false,tour:false,kota2:true,...(s.routePlan.stops||{})};
    ...
    s.routePlan.apiKey=s.routePlan.apiKey||'';
    s.routePlan.favoriteGas=s.routePlan.favoriteGas||'';
    s.routePlan.arrivalMode=s.routePlan.arrivalMode||'buffer';
    s.routePlan.customBuffer=Number(s.routePlan.customBuffer||30);
    s.routePlan.lateAllowance=Number(s.routePlan.lateAllowance||30);
    s.routePlan.trafficBuffer=Number(s.routePlan.trafficBuffer||20);
    s.routePlan.kotaIntervalMin=Number(s.routePlan.kotaIntervalMin||90);
    s.routePlan.maxContinuousDriveMin=Number(s.routePlan.maxContinuousDriveMin||120);
    s.routePlan.sideTripMin=Number(s.routePlan.sideTripMin||20);
    s.routePlan.sideTripKm=Number(s.routePlan.sideTripKm||5);
    s.routePlan.priority={afterExit:'campNear',super:'routeNear',tour:'smallDetour',...(s.routePlan.priority||{})};
    s.routePlan.analysis=s.routePlan.analysis||null;
  ...
  function routeStopDefs(){
    return [
      ...
      {id:'conv',label:'出発前コンビニ',query:'コンビニ',min:10,kind:'出発前',zone:'beforeHighway',note:'自宅から高速入口まで'},
      {id:'sa',label:'SA/PA休憩',query:'サービスエリア パーキングエリア',min:18,kind:'高速',zone:'highway',note:'連続運転2時間以上なら提案'},
      {id:'kota1',label:'コタ散歩休憩',query:'犬 散歩 休憩 公園',min:15,kind:'高速/休憩',zone:'highway',note:'コタ休憩間隔を加味'},
      {id:'afterConv',label:'降りてからコンビニ',query:'コンビニ',min:10,kind:'降りてから',zone:'afterExit',note:'キャンプ場に近い/ルート上を優先'},
      {id:'super',label:'スーパー',query:'スーパー',min:25,kind:'降りてから',zone:'afterExit',note:'寄り道少なめ/品揃え'},
      {id:'michi',label:'道の駅',query:'道の駅',min:20,kind:'降りてから',zone:'afterExit',note:'休憩と買い足し'},
      {id:'tour',label:'観光候補',query:'観光スポット',min:35,kind:'観光',zone:'afterExit',note:'寄り道少なめだけ候補'},
      {id:'kota2',label:'到着前コタ散歩',query:'犬 散歩',min:10,kind:'降りてから',zone:'afterExit',note:'チェックイン前に短く'}
    ];
  }
  ...
  function arrivalBufferMin(mode=state.routePlan?.arrivalMode){
  ...
  function routeNeedsBreak(){return (Number(state.routePlan?.driveMin)||150) >= (Number(state.routePlan?.maxContinuousDriveMin)||120)}
  function routeChosenStops(){return routeStopDefs().filter(d=>state.routeSuggestions?.[d.id]==='go' || (!state.routeSuggestions?.[d.id] && (state.routePlan?.stops?.[d.id] || (d.id==='sa' && routeNeedsBreak()))))}
  function routeStopTotal(){return routeChosenStops().reduce((n,d)=>n+d.min,0)}
  function routeDepartureOption(label,mode,extraBuffer){
    const target=new Date(routeCheckinDate().getTime()-(arrivalBufferMin(mode)+Number(extraBuffer||0))*60000);
    const total=(Number(state.routePlan?.driveMin)||150)+routeStopTotal()+Number(state.routePlan?.trafficBuffer||0);
  ...
  function routeDepartureOptions(){
    return [routeDepartureOption('余裕あり','buffer',30),routeDepartureOption('ぴったり','exact',0),routeDepartureOption('遅れてもOK','late',0)];
  }
  function routeRows(mode=state.routePlan?.arrivalMode||'buffer'){
    const chosen=routeChosenStops(), drive=Number(state.routePlan?.driveMin)||150;
    const per=Math.max(10,Math.round(drive/Math.max(1,chosen.length+1)));
    const opt=routeDepartureOption('採用',mode,0);
    const [dh,dm]=opt.depart.split(':').map(Number);
    let cur=new Date();cur.setHours(dh,dm,0,0);
    const rows=[{id:'home',label:'自宅出発',kind:'出発',time:`${pad(cur.getHours())}:${pad(cur.getMinutes())}`,min:0,query:routeOrigin(),fixed:true}];
    chosen.forEach(d=>{cur=new Date(cur.getTime()+per*60000);rows.push({...d,time:`${pad(cur.getHours())}:${pad(cur.getMinutes())}`,fixed:false});cur=new Date(cur.getTime()+d.min*60000)});
  ...
  function routeGoogleUrl(plan=null){
    const p=plan||{}, origin=p.origin||routeOrigin(), dest=p.destination||routeTarget();
    const stops=(p.selectedStops||routeChosenStops().map(d=>d.query)).slice(0,3).join('|');
    return `https://www.google.com/maps/dir/?api=1&origin=${encodeURIComponent(origin)}&destination=${encodeURIComponent(dest)}&travelmode=driving&waypoints=${encodeURIComponent(stops)}`;
  }
  ...
  function routeArrivalLabel(){
    ...
    if(m==='exact')return 'チェックインぴったり';
    if(m==='late')return `遅れてもOK（+${state.routePlan?.lateAllowance||30}分）`;
    return `${state.routePlan?.customBuffer||30}分前到着`;
  }
  function routeShareText(plan=null){
    const rows=(plan?.rows||routeRows()).map(r=>`${r.time} ${r.label}${r.min?`（${r.min}分）`:''}`).join('\n');
    const chosen=(plan?.selectedLabels||routeChosenStops().map(d=>d.label)).join(' / ')||'なし';
    return `【OUTBASE ルート計画】\n${current().title}\n出発地:${plan?.origin||routeOrigin()}\n目的地:${plan?.destination||routeTarget()}\nチェックイン:${state.routePlan?.checkin||'13:00'}\n到着設定:${routeArrivalLabel()}\n寄り道:${chosen}\n\n${rows}\n\nGoogle Maps:\n${routeGoogleUrl(plan)}`;
  }
  function routeCandidateCard(opt){
  ...
  function routeSegmentHtml(zone,title,caption){
    const defs=routeStopDefs().filter(d=>d.zone===zone);
    return `<details class="routeDetails" open><summary><b>${esc(title)}</b><small>${esc(caption)}</small></summary><div class="routeChips">${defs.map(d=>`<button class="${state.routePlan?.stops?.[d.id]?'active':''}" data-act="toggleRouteStop" data-stop="${d.id}"><b>${esc(d.label)}</b><small>${esc(d.note)} / ${d.min}分</small></button>`).join('')}</div><div class="routeSearchOps">${defs.slice(0,4).map(d=>`<button data-act="searchRouteStop" data-stop="${d.id}">${esc(d.label)}をGoogle Mapsで探す</button>`).join('')}</div></details>`;
  }

  function routeSuggestionDefs(){
    const longDrive=routeNeedsBreak();
    const hasLate=(state.routePlan?.arrivalMode||'buffer')==='late';
    const hasTour=(state.routePlan?.priority?.tour||'smallDetour')==='timeRich';
    return [
      {id:'gas',zone:'beforeHighway',title:'高速に乗る前に給油',tag:'燃料',defaultOn:true,why:'キャンプ場周辺で慌てない。高速前のルート上で探す。',search:'給油候補を見る',min:10},
      {id:'conv',zone:'beforeHighway',title:'高速前コンビニ',tag:'朝食/氷/飲み物',defaultOn:true,why:'朝食、飲み物、氷、忘れ物の最終補充。自宅→高速入口で探す。',search:'コンビニ候補を見る',min:10},
      {id:'sa',zone:'highway',title:longDrive?'SA/PA休憩を入れる':'SA/PAは必要なら入れる',tag:'高速休憩',defaultOn:longDrive,why:longDrive?'連続運転が2時間以上。休憩ポイントを計画に入れる。':'短距離なら必須ではない。眠気やトイレ都合で入れる。',search:'SA/PA候補を見る',min:18},
      {id:'kota1',zone:'highway',title:'コタ休憩ポイント',tag:'コタ',defaultOn:longDrive,why:'長距離ならトイレ・水分・気分転換の候補を持つ。',search:'コタ休憩候補を見る',min:15},
      {id:'afterConv',zone:'afterExit',title:'高速を降りてからコンビニ',tag:'直前補充',defaultOn:true,why:'キャンプ場に近く、寄り道が少ない場所を優先して探す。',search:'コンビニ候補を見る',min:10},
      {id:'super',zone:'afterExit',title:'スーパーは出口後〜キャンプ場寄り',tag:'買い物',defaultOn:true,why:'生鮮・氷・追加食材。寄り道少なめかキャンプ場近くを優先。',search:'スーパー候補を見る',min:25},
      {id:'michi',zone:'afterExit',title:'道の駅は休憩兼ねて候補',tag:'道の駅',defaultOn:false,why:'時間に余裕があれば、トイレ・軽食・地元食材の候補にする。',search:'道の駅候補を見る',min:20},
      {id:'tour',zone:'afterExit',title:hasTour?'観光を1つだけ候補にする':'観光は余裕がある時だけ',tag:'観光',defaultOn:hasTour,why:hasLate?'遅れてもOK設定なら無理に観光を入れない。':'高速出口からキャンプ場までで、寄り道少なめだけ候補にする。',search:'観光候補を見る',min:35},
      {id:'kota2',zone:'afterExit',title:'到着前コタ散歩',tag:'コタ',defaultOn:true,why:'受付前後に落ち着けるよう、短い散歩候補を残す。',search:'コタ散歩候補を見る',min:10}
    ];
  }
  function routeSuggestionState(id,def){
    const raw=state.routeSuggestions?.[id];
    if(raw==='go'||raw==='skip')return raw;
    return (state.routePlan?.stops?.[id] || def.defaultOn)?'go':'skip';
  }
  function routeSuggestionSectionHtml(zone,title,caption){
    const defs=routeSuggestionDefs().filter(d=>d.zone===zone);
    return `<details class="routeSuggestGroup" open><summary><b>${esc(title)}</b><small>${esc(caption)}</small></summary>
      <div class="routeSuggestGrid">${defs.map(d=>{
        const st=routeSuggestionState(d.id,d);
        return `<article class="routeSuggest ${st==='go'?'go':'skip'}" data-stop="${d.id}">
          <div class="routeSuggestTop"><span><b>${esc(d.title)}</b><small>${esc(d.tag)} / ${d.min}分</small></span><em>${st==='go'?'寄る候補':'今回は寄らない'}</em></div>
          <p>${esc(d.why)}</p>
          <div class="routeSuggestOps">
            <button data-act="acceptRouteSuggestion" data-stop="${d.id}">ここに寄る</button>
            <button data-act="skipRouteSuggestion" data-stop="${d.id}">今回は寄らない</button>
            <button data-act="searchRouteStop" data-stop="${d.id}">${esc(d.search)}</button>
          </div>
        </article>`;
      }).join('')}</div>
    </details>`;
  ...
  function routeCandidatesHtml(){
    ...
    return `<details class="routeDetails" open><summary><b>店名が決まったら保存</b><small>基本ルート保存後、OUTBASE提案で「寄る」を選び、実店舗名が分かったらここに残す。</small></summary>
      ...
      <button class="routeSaveCandidate" data-act="saveRouteCandidate">候補をOUTBASEに保存</button>
      <div class="routeCandidateList">${list.length?list.map(c=>`<div><span><b>${esc(c.name)}</b><small>${esc(c.kind)} / ${esc(c.time)}</small></span><button data-act="deleteRouteCandidate" data-id="${c.id}">削除</button></div>`).join(''):'<p>保存候補はまだない。</p>'}</div>
    </details>`;
  }
  function savedRouteHtml(){
    const plans=state.savedRoutePlans||[];
    return `<section class="section savedRoutes"><div class="head"><b>保存済みルート</b><small>当日はこのままGoogle Mapsナビ＋OUTBASEドライブ記録へ。</small></div>${plans.length?plans.map(p=>`<div class="savedRouteCard"><span><b>${esc(p.name)}</b><small>${esc(p.depart)}出発 / ${esc(p.arrival)}到着目標 / ${esc(p.createdAt)}</small></span><div><button data-act="useSavedRoute" data-id="${p.id}">このルートで出発</button><button data-act="openSavedRoute" data-id="${p.id}">Maps</button><button data-act="deleteSavedRoute" data-id="${p.id}">削除</button></div></div>`).join(''):`<div class="routeNote">保存済みルートはまだない。</div>`}</section>`;
  }
...
${starterPanelHtml()}
  ...
  function render(keepScroll=true){
    const sx=window.scrollX||0, sy=Number.isFinite(state.__scrollY)?state.__scrollY:scrollYNow();
    try{
      ...
      if(state.routeFocus){
        ...
        save();
        setTimeout(()=>{
          const el=document.querySelector(`[data-stop="${routeFocus}"]`)||document.querySelector('.routePlanner');
          if(el&&el.scrollIntoView)el.scrollIntoView({block:'center'});
        },0);
      }else if(keepScroll)restoreScroll(sy);
      else{state.__scrollY=0;setTimeout(()=>window.scrollTo(0,0),0)}
    }catch(err){
      console.error(err);
  ...
  function bind(){
    ...
    document.querySelectorAll('[data-route-field]').forEach(el=>{
      const update=()=>{setRouteField(el.dataset.routeField,el.type==='number'?Number(el.value):el.value,false)};
      el.oninput=update;
  ...
  function act(a,el){
    ...
    if(a==='markDriveChecked')return markDriveChecked();
    ...
    if(a==='deleteSavedRoute')return deleteSavedRoute(el.dataset.id);
    if(a==='saveRouteCandidate')return saveRouteCandidate();
    if(a==='deleteRouteCandidate')return deleteRouteCandidate(el.dataset.id);
    if(a==='acceptRouteSuggestion')return setRouteSuggestion(el.dataset.stop,'go');
    if(a==='skipRouteSuggestion')return setRouteSuggestion(el.dataset.stop,'skip');
  ...
  function setRouteField(k,v,doRender=false){
    state.routePlan=state.routePlan||{};
    state.routePlan[k]=v;
    if(k==='origin')state.settings.home=v;
    save();
    if(doRender)render();
  }
  ...
  function setRouteSuggestion(id,mode){
    ...
    state.routePlan=state.routePlan||{};
    state.routePlan.stops={gas:true,conv:true,sa:true,kota1:true,afterConv:true,super:true,michi:false,tour:false,kota2:true,...(state.routePlan.stops||{})};
    state.routeSuggestions[id]=mode==='go'?'go':'skip';
    state.routePlan.stops[id]=mode==='go';
    state.routeFocus=id;
    save();render();
  }
  ...
  function saveRouteCandidate(){
    const name=String(state.routePlan?.candidateName||'').trim();
    if(!name){toast('候補名を入力');return}
    const kind=state.routePlan?.candidateKind||'候補';
    state.routeCandidates=[{id:uid('cand'),name,kind,time:new Date().toLocaleString('ja-JP')},...(state.routeCandidates||[])].slice(0,40);
    state.routePlan.candidateName='';
    save();render();toast('候補保存');
  }
  function deleteRouteCandidate(id){
    state.routeCandidates=(state.routeCandidates||[]).filter(c=>c.id!==id);
    save();render();toast('候補削除');
  }
  ...
  function snapshotRoutePlan(){
    const opt=routeDepartureOption('採用',state.routePlan?.arrivalMode||'buffer',0);
    const rows=routeRows();
    return {id:uid('route'),eventId:current().id,name:`${current().title} ${opt.depart}出発`,origin:routeOrigin(),destination:routeTarget(),checkin:state.routePlan?.checkin||'13:00',arrivalMode:state.routePlan?.arrivalMode||'buffer',arrivalLabel:routeArrivalLabel(),depart:opt.depart,arrival:opt.arrival,total:opt.total,rows,selectedStops:routeChosenStops().map(d=>d.query),selectedLabels:routeChosenStops().map(d=>d.label),url:routeGoogleUrl(),analysis:state.routePlan?.analysis||null,createdAt:new Date().toLocaleString('ja-JP')};
  }
  function saveRoutePlan(){
    const plan=snapshotRoutePlan();
    state.savedRoutePlans=[plan,...(state.savedRoutePlans||[]).filter(p=>p.eventId!==plan.eventId||p.name!==plan.name)].slice(0,20);
    save();render();toast('ルート保存');
  }
  function findSavedRoute(id){return (state.savedRoutePlans||[]).find(p=>p.id===id)}
  function openSavedRoute(id){const p=findSavedRoute(id); if(p)window.open(p.url||routeGoogleUrl(p),'_blank')}
  function deleteSavedRoute(id){state.savedRoutePlans=(state.savedRoutePlans||[]).filter(p=>p.id!==id);save();render();toast('削除')}
  function useSavedRoute(id){
    const p=findSavedRoute(id); if(!p)return;
    state.activeRoutePlanId=id;state.route='field';state.fieldFixedMode='drive';state.fieldMode='drive';
    addPublicRecord('memo','このルートで出発',`${p.depart}出発 / ${p.destination}`);
    save();render(false);setTimeout(()=>window.open(p.url||routeGoogleUrl(p),'_blank'),200);
  }
  function loadGoogleMapsRouteApi(){
    const key=state.routePlan?.apiKey;
    if(!key)return Promise.reject(new Error('APIキー未設定'));
    if(window.google?.maps?.DirectionsService)return Promise.resolve();
    return new Promise((resolve,reject)=>{
      const cb='outbaseMapsReady_'+Date.now();
      ...
      s.onerror=()=>reject(new Error('Google Maps API読込失敗'));
      document.head.appendChild(s);
    });
  }
  function cleanInstruction(s){return String(s||'').replace(/<[^>]+>/g,'').replace(/\s+/g,' ').trim()}
  function directionsRequest(avoidHighways){
    const svc=new google.maps.DirectionsService();
    return new Promise((resolve,reject)=>{
      svc.route({origin:routeOrigin(),destination:routeTarget(),travelMode:google.maps.TravelMode.DRIVING,avoidHighways:!!avoidHighways,provideRouteAlternatives:true},(res,status)=>{if(status==='OK')resolve(res); else reject(new Error(status))});
    });
  }
  function parseDirections(res,type){
    const route=res.routes?.[0]||{}, leg=route.legs?.[0]||{};
    const steps=(leg.steps||[]).map(s=>({text:cleanInstruction(s.instructions),distance:s.distance?.text||'',duration:s.duration?.text||''}));
  ...
  /* RESTORE03.5: 予定連動・APIなし出発逆算 */
  function campRouteEvents(){
    return (state.events||[]).filter(e=>e.type==='camp' || /キャンプ|camp|スノーピーク|CF|フィールド/i.test((e.title||'')+(e.place||'')));
  }
  function activeCampForRoute(){
    const camps=campRouteEvents();
    return camps.find(e=>e.id===state.currentPlanId)||camps[0]||current();
  }
  function planDestination(e=activeCampForRoute()){
    return e?.place || e?.title || state.routePlan?.destination || 'キャンプ場';
  }
  function syncRouteFromPlan(id){
    ...
    if(e.checkin)state.routePlan.checkin=e.checkin;
  }
  function routeOrigin(){
    return state.settings?.home || state.routePlan?.origin || '千葉県柏市';
  }
  function routeTarget(){
    return state.routePlan?.destination || planDestination();
  }
  function setRouteField(k,v,doRender=false){
    state.routePlan=state.routePlan||{};
    if(k==='origin'||k==='home'){
  ...
  function routeArrivalLabel(){
    const v=state.routePlan?.arrivalTarget||'b30';
    if(v==='exact')return 'チェックインちょうど';
    if(v==='b15')return '15分前到着';
    if(v==='b30')return '30分前到着';
    if(v==='b60')return '1時間前到着';
    if(v==='late')return '少し遅れてもOK';
    return `${state.routePlan?.customBuffer||30}分前到着`;
  }
  function simpleArrivalBufferFor(mode){
    const v=state.routePlan?.arrivalTarget||'b30';
  ...
  function routePlanSelectHtml(){
    const camps=campRouteEvents();
    if(!camps.length)return `<div class="routePlanCard"><b>予定が未登録</b><small>先に予定へキャンプ場を登録すると、ここに反映される。</small></div>`;
    return `<label><span>予定から選ぶ</span><select data-route-field="currentPlanId">
      ${camps.map(e=>`<option value="${e.id}" ${e.id===(activeCampForRoute()?.id||state.currentPlanId)?'selected':''}>${esc(e.title)}${e.start?` / ${esc(e.start)}`:''}</option>`).join('')}
    </select></label>`;
  }
  function driveCheckedText(){
    return state.routePlan?.driveCheckedAt || '未記録';
  }
  function markDriveChecked(){
    state.routePlan.driveCheckedAt=new Date().toLocaleString('ja-JP');
    save();render();
    window.open(simpleRouteGoogleUrl(),'_blank');
  }
  function routePrepHero(){
    const opt=simpleDepartOption('標準',state.routePlan?.arrivalMode||'buffer',0);
    return `<section class="prepHero routeOnlyHero simpleRouteHero">
      <div class="prepHeroTop">
        <span><b>出発時間を決める</b><small>予定のキャンプ場とチェックインを使い、当日想定の移動時間＋寄り道時間で逆算する。</small></span>
        <span class="prepScore routeScoreMini">${opt.depart}</span>
      </div>
      <div class="prepCommand">
        <button class="mainCmd" data-act="openSimpleRoute">Google Mapsで当日想定を確認</button>
        <button class="subCmd" data-act="saveSimpleRoute">計画保存</button>
      </div>
    </section>`;
  }
  function routePrepView(){
    ...
    const optC=simpleDepartOption('ギリギリ','exact',0);
    const rows=simpleRouteRows();
    return `<section class="section simpleRoute route035">
      <div class="simpleRouteLead">
        <b>APIなし出発逆算</b>
        <p>自宅は登録制。キャンプ場とチェックインは予定から選ぶ。Google Mapsの当日想定時間だけ入力し、OUTBASEは出発時刻を計算して保存する。</p>
      </div>

      <div class="routeFormGrid routeBasics">
        <label><span>登録済み自宅</span><input data-route-field="home" value="${esc(routeOrigin())}" placeholder="例：千葉県柏市"></label>
        ${routePlanSelectHtml()}
        <label><span>予定のキャンプ場</span><input value="${esc(routeTarget())}" readonly></label>
        <label><span>チェックイン</span><input data-route-field="checkin" type="time" value="${esc(state.routePlan?.checkin||'13:00')}"></label>
      </div>

      <section class="routeDriveTime">
        <div>
          <b>当日想定の移動時間</b>
          <p>Google Maps側でキャンプ当日の出発/到着日時を指定して確認し、その分数をここに入れる。OUTBASE側では自動取得しない。</p>
        </div>
        <div class="routeDriveOps">
          <button data-act="markDriveChecked">Google Mapsで確認</button>
          <label><span>移動時間（分）</span><input data-route-field="driveMin" type="number" value="${Number(state.routePlan?.driveMin||150)}"></label>
        </div>
        <small>確認記録：${esc(driveCheckedText())}</small>
      </section>

      <div class="routeFormGrid routeTargets">
        <label><span>到着目標</span><select data-route-field="arrivalTarget">
          <option value="exact" ${state.routePlan?.arrivalTarget==='exact'?'selected':''}>チェックインちょうど</option>
          ...
          <option value="late" ${state.routePlan?.arrivalTarget==='late'?'selected':''}>少し遅れてもOK</option>
        </select></label>
        <label><span>追加予備（任意）</span><input data-route-field="trafficBuffer" type="number" value="${Number(state.routePlan?.trafficBuffer||20)}"></label>
      </div>
      <div class="routeNote">追加予備は、Google Mapsの移動時間に入らないトイレ・コタ・受付前の余白。渋滞はGoogle Maps側で当日時刻を指定して確認する前提。</div>
      ...
      <section class="simpleCalc">
        <div><span>移動</span><b>${Number(state.routePlan?.driveMin||150)}分</b></div>
        <div><span>寄り道</span><b>${simpleStopTotal()}分</b></div>
        <div><span>追加予備</span><b>${Number(state.routePlan?.trafficBuffer||0)}分</b></div>
        <div class="total"><span>自宅出発</span><b>${optB.depart}</b></div>
      </section>
      ...
      <div class="routeNote">APIなし版では、Google Mapsの移動時間は自動取得しない。予定連動・出発逆算・寄り道時間・保存・当日ナビ連携をOUTBASEで担当する。</div>
    </section>${savedRouteHtml()}`;
  }
  function snapshotRoutePlan(){
    const opt=simpleDepartOption('採用',state.routePlan?.arrivalMode||'buffer',0);
    const rows=simpleRouteRows();
    return {
      id:uid('route'), eventId:activeCampForRoute()?.id||current().id, name:`${activeCampForRoute()?.title||current().title} ${opt.depart}出発`,
      origin:routeOrigin(), destination:routeTarget(), checkin:state.routePlan?.checkin||'13:00',
      arrivalMode:state.routePlan?.arrivalMode||'buffer', arrivalLabel:routeArrivalLabel(),
      depart:opt.depart, arrival:opt.arrival, total:opt.total, rows,
      selectedStops:simpleStopDefs().filter(d=>simpleStopState(d.id).on).map(d=>`${d.label} ${simpleStopState(d.id).min}分`),
      selectedLabels:simpleStopDefs().filter(d=>simpleStopState(d.id).on).map(d=>d.label),
      url:simpleRouteGoogleUrl(), mode:'api-less-plan-linked', driveCheckedAt:state.routePlan?.driveCheckedAt||'', createdAt:new Date().toLocaleString('ja-JP')
    };
  }
  function saveSimpleRoutePlan(){return saveRoutePlan()}
  ...
  /* RESTORE03.6: APIなしMVP整理 */
  function currentSavedRoutePlan(){
    const ev=activeCampForRoute();
    const eid=ev?.id||current().id;
    return (state.savedRoutePlans||[]).find(p=>p.eventId===eid) || null;
  }
  function apiLessStepHeaderHtml(){
    return `<div class="routeFlow">
      <div class="done"><b>1</b><span>予定</span><small>予定から選ぶ</small></div>
      <div class="done"><b>2</b><span>移動</span><small>当日想定分</small></div>
      <div class="now"><b>3</b><span>寄り道</span><small>時間だけ足す</small></div>
      <div class="goal"><b>4</b><span>出発</span><small>保存して当日へ</small></div>
    </div>`;
  }
  function savedRouteStrongHtml(){
    const p=currentSavedRoutePlan();
    if(!p)return `<section class="section savedRouteStrong empty"><div><b>保存済みの出発計画はまだない</b><small>移動時間と寄り道を決めたら「この計画を保存」。当日はここから出発する。</small></div></section>`;
    return `<section class="section savedRouteStrong">
      <div><b>保存済み出発計画</b><small>${esc(p.destination)} / ${esc(p.createdAt||'')}</small></div>
      <div class="savedRouteTime"><span>自宅出発</span><b>${esc(p.depart)}</b></div>
      <div class="savedRouteOps"><button data-act="useSavedRoute" data-id="${p.id}">この計画で出発</button><button data-act="openSavedRoute" data-id="${p.id}">Google Maps確認</button></div>
    </section>`;
  }
  function routePrepHero(){
    const opt=simpleDepartOption('標準',state.routePlan?.arrivalMode||'buffer',0);
    return `<section class="prepHero routeOnlyHero simpleRouteHero">
      <div class="prepHeroTop"><span><b>出発時間を決める</b><small>APIなし版。予定・当日想定の移動時間・寄り道時間だけで、自宅出発を決める。</small></span><span class="prepScore routeScoreMini">${opt.depart}</span></div>
      <div class="prepCommand"><button class="mainCmd" data-act="openSimpleRoute">Google Mapsで当日想定を確認</button><button class="subCmd" data-act="saveSimpleRoute">この計画を保存</button></div>
    </section>`;
  }
  function routePrepView(){
    const plan=activeCampForRoute();
    ...
    const optC=simpleDepartOption('ギリギリ','exact',0);
    const rows=simpleRouteRows();
    return `<section class="section simpleRoute route036">
      <div class="simpleRouteLead"><b>APIなしルート計画</b><p>ルート検索アプリではなく、キャンプ当日の「何時に家を出るか」を決める道具。細かい店探しと渋滞はGoogle Maps、OUTBASEは予定連動・逆算・保存・当日記録を担当。</p></div>
      ${apiLessStepHeaderHtml()}
      ${savedRouteStrongHtml()}
      <section class="routePanel"><div class="routePanelHead"><b>1. 予定と自宅</b><small>自宅は登録制。キャンプ場とチェックインは予定から反映。</small></div><div class="routeFormGrid routeBasics compact"><label><span>登録済み自宅</span><input data-route-field="home" value="${esc(routeOrigin())}" placeholder="例：千葉県柏市"></label>${routePlanSelectHtml()}<label><span>予定のキャンプ場</span><input value="${esc(routeTarget())}" readonly></label><label><span>チェックイン</span><input data-route-field="checkin" type="time" value="${esc(state.routePlan?.checkin||'13:00')}"></label></div></section>
      <section class="routeDriveTime compact"><div><b>2. 当日想定の移動時間</b><p>Google Maps側でキャンプ当日の日時を指定して確認し、表示された時間を分で入れる。</p></div><div class="routeDriveOps"><button data-act="markDriveChecked">Google Mapsで確認</button><label><span>移動時間（分）</span><input data-route-field="driveMin" type="number" value="${Number(state.routePlan?.driveMin||150)}"></label></div><small>確認記録：${esc(driveCheckedText())}</small></section>
      <section class="routePanel"><div class="routePanelHead"><b>3. 到着目標と寄り道</b><small>寄り道は行く/行かない＋所要時間だけ。</small></div><div class="routeFormGrid routeTargets compact"><label><span>到着目標</span><select data-route-field="arrivalTarget"><option value="exact" ${state.routePlan?.arrivalTarget==='exact'?'selected':''}>チェックインちょうど</option><option value="b15" ${state.routePlan?.arrivalTarget==='b15'?'selected':''}>15分前</option><option value="b30" ${(state.routePlan?.arrivalTarget||'b30')==='b30'?'selected':''}>30分前</option><option value="b60" ${state.routePlan?.arrivalTarget==='b60'?'selected':''}>1時間前</option><option value="late" ${state.routePlan?.arrivalTarget==='late'?'selected':''}>少し遅れてもOK</option></select></label><label><span>追加予備（分）</span><input data-route-field="trafficBuffer" type="number" value="${Number(state.routePlan?.trafficBuffer||20)}"></label></div><div class="simpleStopGrid compactStops">${simpleStopDefs().map(d=>{const s=simpleStopState(d.id);return `<article class="simpleStop ${s.on?'on':'off'}" data-stop="${d.id}"><button class="simpleStopMain" data-act="toggleSimpleStop" data-stop="${d.id}"><b>${esc(d.label)}</b><small>${esc(d.hint)}</small><em>${s.on?'行く':'行かない'}</em></button><label><span>分</span><input data-simple-min="${d.id}" type="number" value="${Number(s.min||d.defaultMin)}"></label></article>`;}).join('')}</div></section>
      <section class="simpleCalc routeResult"><div><span>移動</span><b>${Number(state.routePlan?.driveMin||150)}分</b></div><div><span>寄り道</span><b>${simpleStopTotal()}分</b></div><div><span>追加予備</span><b>${Number(state.routePlan?.trafficBuffer||0)}分</b></div><div class="total"><span>自宅出発</span><b>${optB.depart}</b></div></section>
      <div class="routeDepartures simpleDepartures compactDepartures">${[optA,optB,optC].map(o=>`<button class="routeDeparture ${o.mode===(state.routePlan?.arrivalMode||'buffer')?'active':''}" data-act="setArrivalMode" data-mode="${o.mode}"><b>${esc(o.depart)}</b><span>${esc(o.label)}</span><small>到着目標 ${esc(o.arrival)} / 合計 ${o.total}分</small></button>`).join('')}</div>
      <details class="routeDetails simplePlanDetail"><summary><b>計画の内訳</b><small>出発から到着目標まで</small></summary><div class="routeTimeline simpleTimeline">${rows.map(r=>`<div class="routeStep fixed"><time>${esc(r.time)}</time><span><b>${esc(r.label)}</b><small>${esc(r.memo)}</small></span><em>計画</em></div>`).join('')}</div></details>
  ...
  /* RESTORE04.5: ルート1画面整理 */
  function ob045CampEvents(){
    return (state.events||[]).filter(e=>{
      const t=`${e.title||''} ${e.type||''} ${e.category||''}`;
      if(/候補|探し|検索|下見|散歩/.test(t))return false;
      return /キャンプ|CF|camp|Camp|赤城|鹿沼|山中湖|月館|にこにこ|ほったらかし|白州|那須|九十九里|ふもとっぱら|まほろば|たきっぱら/.test(t);
    });
  }
  function ob045PlanSelect(){
    const list=ob045CampEvents();
    const active=activeCampForRoute?.();
    const items=list.length?list:(active?[active]:(state.events||[]).filter(e=>!/候補|探し|検索|下見|散歩/.test(`${e.title||''}`)).slice(0,1));
    return `<label class="obPlanSelectMini"><span>予定</span><select data-route-field="currentPlanId">
      ${items.map(e=>`<option value="${esc(e.id)}" ${e.id===state.currentPlanId?'selected':''}>${esc(e.title||'キャンプ予定')}${e.start?` / ${esc(e.start)}`:''}</option>`).join('')}
    </select></label>`;
  }
  function ob045PlanLine(){
    const d=ob043CampDate?.()||'';
    return `${esc(ob043CampTitle?.()||routeTarget())}${d?` / ${esc(d)}`:''} / CI ${esc(state.routePlan?.checkin||'13:00')}`;
  }
  function ob045SavedPlan(){
    const p=currentSavedRoutePlan?.();
    if(!p)return `<div class="obSaved045 muted"><span>未保存</span><b>保存するとこの予定に紐づく</b></div>`;
    return `<div class="obSaved045"><span>保存済み</span><b>${esc(p.depart)}出発</b><button data-act="useSavedRoute" data-id="${p.id}">出発</button><button data-act="openSavedRoute" data-id="${p.id}">Maps</button></div>`;
  }
  function ob045StopButtons(){
    return simpleStopDefs().map(d=>{
      const s=simpleStopState(d.id);
      return `<button class="obStopMini ${s.on?'on':'off'}" data-act="toggleSimpleStop" data-stop="${d.id}">
        <b>${esc(d.label)}</b><span>${s.on?`${Number(s.min||d.defaultMin)}分`:'OFF'}</span>
      </button>`;
    }).join('');
  }
  function ob045HomeEdit(){
    return `<label class="obMiniField wide"><span>自宅</span><input data-route-field="home" value="${esc(routeOrigin())}" placeholder="例：千葉県柏市"></label>`;
  }
  routePrepView = function(){
    const plan=activeCampForRoute?.();
    if(plan && !state.routePlan.destination)syncRouteFromPlan(plan.id);
    ...
    const rows=simpleRouteRows();
    const at=state.routePlan?.arrivalTarget||'b30';
    return `<section class="obRouteDash obRoute045">
      <div class="obHeroCompact">
        <div class="obHeroCompactMain">
          <span class="label">自宅出発</span>
          <strong>${standard.depart}</strong>
          <em>${routeArrivalLabel()}</em>
        </div>
        <p>${ob045PlanLine()}</p>
        <div class="obHeroMiniStats">
          <span>移動<b>${Number(state.routePlan?.driveMin||150)}分</b></span>
          <span>寄り道<b>${simpleStopTotal()}分</b></span>
          <span>予備<b>${Number(state.routePlan?.trafficBuffer||0)}分</b></span>
        </div>
        <div class="obHeroCompactActions">
          <button class="primary" data-act="saveSimpleRoute">保存</button>
          <button data-act="openSimpleRoute">Maps</button>
        </div>
      </div>
      ...
      <div class="obStopMiniGrid">
        ${ob045StopButtons()}
      </div>

      <div class="obDepartMiniBand">
        <button class="${at==='b60'?'active':''}" data-act="setArrivalTarget" data-target="b60"><span>余裕あり</span><b>${roomy.depart}</b></button>
        <button class="${at==='b30'||at==='b15'?'active':''}" data-act="setArrivalTarget" data-target="b30"><span>標準</span><b>${standard.depart}</b></button>
        <button class="${at==='exact'?'active':''}" data-act="setArrivalTarget" data-target="exact"><span>ギリギリ</span><b>${tight.depart}</b></button>
      </div>

      <details class="obEdit obEditCompact">
        <summary><b>調整</b><small>移動時間・到着目標・自宅</small></summary>
        <div class="obEditGrid">
          ${ob045HomeEdit()}
          <label class="obMiniField"><span>チェックイン</span><input data-route-field="checkin" type="time" value="${esc(state.routePlan?.checkin||'13:00')}"></label>
          <label class="obMiniField"><span>当日想定移動</span><input data-route-field="driveMin" type="number" value="${Number(state.routePlan?.driveMin||150)}"></label>
  ...
  /* RESTORE04.8: ルート余白・停止候補・出発計算連動修正 */
  simpleStopDefs = function(){
    return [
      {id:'gas',label:'給油',hint:'高速前か出発直後',defaultMin:10},
      {id:'conv',label:'コンビニ',hint:'朝食・氷・飲み物',defaultMin:10},
      {id:'super',label:'買物',hint:'買い足し・生鮮',defaultMin:25},
      {id:'sa',label:'SA/PA休憩',hint:'トイレ・休憩',defaultMin:20},
      {id:'tour',label:'観光',hint:'寄り道ありの日だけ',defaultMin:45}
    ];
  }
  function ob048ArrivalBuffer(target){
    if(target==='exact')return 0;
    if(target==='late')return -Number(state.routePlan?.lateAllowance||30);
    if(target==='b15')return 15;
    if(target==='b60')return 60;
    return 30;
  }
  function ob048RouteTotal(){
    return (Number(state.routePlan?.driveMin)||150)+simpleStopTotal()+Number(state.routePlan?.trafficBuffer||0);
  }
  function ob048DepartForTarget(target,label){
    const check=routeCheckinDate();
    const buffer=ob048ArrivalBuffer(target);
    const arrival=new Date(check.getTime()-buffer*60000);
    const dep=new Date(arrival.getTime()-ob048RouteTotal()*60000);
    return {
      target,
      label,
      buffer,
      depart:`${pad(dep.getHours())}:${pad(dep.getMinutes())}`,
      arrival:`${pad(arrival.getHours())}:${pad(arrival.getMinutes())}`,
      total:ob048RouteTotal()
    };
  }
  routeArrivalLabel = function(){
    const target=state.routePlan?.arrivalTarget||'b30';
    if(target==='exact')return 'チェックインちょうど';
    if(target==='late')return '少し遅れてOK';
    if(target==='b15')return '15分前到着';
    if(target==='b60')return '1時間前到着';
    return '30分前到着';
  }
  simpleDepartOption = function(label,mode,extra=0){
    // Legacy compatibility. RESTORE04.8の表示は arrivalTarget 固定値で計算する。
    const target = mode==='exact' ? 'exact' : (mode==='late' ? 'late' : (Number(extra||0)>=30 ? 'b60' : (state.routePlan?.arrivalTarget||'b30')));
    const o=ob048DepartForTarget(target,label);
    return {label,mode,depart:o.depart,arrival:o.arrival,total:o.total};
  }
  simpleRouteRows = function(){
    const opt=ob048DepartForTarget(state.routePlan?.arrivalTarget||'b30','採用');
    return [
      {time:opt.depart,label:'自宅出発',memo:`移動 ${Number(state.routePlan?.driveMin)||150}分 / 寄り道 ${simpleStopTotal()}分 / 予備 ${Number(state.routePlan?.trafficBuffer||0)}分`},
      ...simpleStopDefs().filter(d=>simpleStopState(d.id).on).map(d=>({time:'＋',label:d.label,memo:`${simpleStopState(d.id).min}分 / ${d.hint}`})),
      {time:opt.arrival,label:'到着目標',memo:routeArrivalLabel()}
    ];
  }
  function ob048PlanLine(){
    const d=ob043CampDate?.()||'';
    return `${esc(ob043CampTitle?.()||routeTarget())}${d?` / ${esc(d)}`:''} / CI ${esc(state.routePlan?.checkin||'13:00')}`;
  }
  routePrepView = function(){
    ...
    if(plan && !state.routePlan.destination)syncRouteFromPlan(plan.id);
    const at=state.routePlan?.arrivalTarget||'b30';
    const current=ob048DepartForTarget(at,'採用');
    const roomy=ob048DepartForTarget('b60','余裕あり');
    const standard=ob048DepartForTarget('b30','標準');
    const tight=ob048DepartForTarget('exact','ギリギリ');
    const rows=simpleRouteRows();
    return `<section class="obRouteDash obRoute045 obRoute048">
      <div class="obHeroCompact">
        <div class="obHeroCompactMain">
          <span class="label">自宅出発</span>
          <strong>${current.depart}</strong>
          <em>${routeArrivalLabel()}</em>
        </div>
        <p>${ob048PlanLine()}</p>
        <div class="obHeroMiniStats">
  ...
  /* RESTORE04.11: 実装前総合監査反映 / 入口整理 / プラン文脈検証版
     正ベース: RESTORE04.8（ルートAPIなしOK部分を維持）
     目的:
       - 使いたい機能に迷わず入れること
       - プランチップは「主役表示」であって機能入口にしないこと
       - キャンプ/散歩/未選択で表示文脈を混ぜないこと
       - 04.9/04.10の未承認積み増しを使わず、必要部品だけを再実装すること
  */
  const OUTBASE_RESTORE0411_AUDIT_LOCK = Object.freeze({
    approvedBase:'RESTORE04.8 route API-less MVP',
    zipPolicy:'監査→設計→実装→node --check→シナリオ検証→ZIP作成',
    selectableTypes:['camp','walk'],
    bottomNav:['予定','探す','準備','記録','思い出'],
    routeRule:'キャンプ選択中だけキャンプ用ルートを表示する。散歩ではチェックイン/SA/PA/場内を出さない。',
    recordRule:'記録入口は 話す / 撮る / 場所 / メモ に集約する。詳細機能は閉じる。'
  });

  function ob411IsSelectablePlan(e){return !!e && (e.type==='camp'||e.type==='walk')}
  function ob411VisiblePlans(){return (state.events||[]).filter(ob411IsSelectablePlan)}
  function ob411FallbackPlan(){return ob411VisiblePlans()[0] || null}
  function ob411CurrentPlan(){
    const cur=(state.events||[]).find(e=>e.id===state.currentPlanId);
    if(ob411IsSelectablePlan(cur))return cur;
    return ob411FallbackPlan() || {id:'inbox',title:'未確認箱',type:'memo',start:'',end:'',place:'',memo:'プラン未選択の仮保存先'};
  }
  function ob411CurrentType(){return ob411CurrentPlan()?.type || 'none'}
  function ob411IsCamp(){return ob411CurrentType()==='camp'}
  function ob411IsWalk(){return ob411CurrentType()==='walk'}
  function ob411SaveTarget(){
    const p=ob411CurrentPlan();
    return ob411IsSelectablePlan(p) ? {id:p.id,title:p.title,type:p.type,inbox:false} : {id:'inbox',title:'未確認箱',type:'memo',inbox:true};
  }
  function ob411PlanKindLabel(p=ob411CurrentPlan()){return p.type==='camp'?'キャンプ':p.type==='walk'?'散歩':'未確認'}
  function ob411RepairState(){
    let changed=false;
    const before=(state.events||[]).length;
    state.events=(state.events||[]).filter(e=>!(e.id==='pay1' && e.title==='カード支払い確認'));
    if(state.events.length!==before)changed=true;
    if(!ob411IsSelectablePlan((state.events||[]).find(e=>e.id===state.currentPlanId))){
      const next=ob411FallbackPlan();
      if(next){state.currentPlanId=next.id;changed=true;}
    }
    if(state.route==='prep' && ob411IsWalk() && state.prepTab==='route'){state.prepTab='overview';changed=true;}
    if(state.route!=='calendar' && state.route!=='discover' && state.route!=='prep' && state.route!=='field' && state.route!=='memory' && state.route!=='home'){
      state.route='home';changed=true;
    }
    if(changed)save();
  }
  ob411RepairState();

  current = function(){return ob411CurrentPlan()}
  sortedPlans = function(){
    ...
    return ob411VisiblePlans().slice().sort((a,b)=>rank(a)-rank(b));
  }
  planProgress = function(e=current()){
    const id=e?.id||state.currentPlanId;
    const rec=planRecords(id).length;
    const st=prepStats(), ps=petPrepStats();
    if(e?.type==='walk')return {score:rec?80:35,records:rec,reviews:planReviews(id).length,prep:0,pet:ps.score};
    const score=Math.round((st.score + (rec?100:25) + ps.score)/3);
    return {score,records:rec,reviews:planReviews(id).length,prep:st.score,pet:ps.score};
  }
  planSwitchCard = function(e){
    const cur=e.id===state.currentPlanId;
    const p=planProgress(e);
    const isCamp=e.type==='camp';
    return `<div class="planSwitchCard ${cur?'current':''}">
      <div class="planSwitchTop"><span><b>${esc(e.title)}</b><small>${esc(isCamp?'キャンプ':'散歩')} / ${esc(planSpan(e))} / ${esc(e.place||'場所未設定')}</small></span><span class="pill ${cur?'dark':'wood'}">${cur?'主役':(isCamp?'キャンプ':'散歩')}</span></div>
      <div class="planMiniStats"><span>使う画面<br>${isCamp?'準備/ルート':'記録/GPS'}</span><span>記録<br>${p.records}</span><span>保存先<br>${cur?'現在':'候補'}</span><span>表示<br>${isCamp?'キャンプ用':'散歩用'}</span></div>
  ...
  planSwitchHtml = function(){
    ...
    return `<div class="form"><div class="head"><div><h2>主役プラン切替</h2><p>ここは保存先と表示文脈の切替。機能を探す入口は下バー。</p></div><button class="btn" type="button" data-act="close">閉じる</button></div>
      <div class="planSwitchList">${plans.map(planSwitchCard).join('') || `<div class="honestBox"><b>主役プランなし</b><p>記録は未確認箱に仮保存して、あとで整理する。</p></div>`}</div>
  ...
  buildPlanContext = function(){
    const p=current(), pr=planProgress(p);
    return `【OUTBASE 主役プラン】\n${p.title}\n${planSpan(p)} / ${ob411PlanKindLabel(p)}\n場所:${p.place||'未設定'}\n記録:${pr.records}\n\n${p.memo||''}`;
  }

  top = function(){
    const p=current();
    return `<div class="top obTop046 obTop0411">
      <div class="top-row">
        <button class="brand" data-route="home"><span class="logo">OB</span><span><b>OUTBASE</b><small>field system</small></span></button>
        <button class="plan" data-act="planSwitch"><i></i><b>${esc(p.title||'未確認箱')}</b><small>${esc(planSpan(p))} / ${esc(ob411PlanKindLabel(p))}</small></button>
      </div>
    </div>`;
  }
  nav = function(){
    const items=[['calendar','▦','予定'],['discover','⌕','探す'],['prep','◫','準備'],['field','＋','記録'],['memory','○','思い出']];
    return `<nav class="bottomNav obNav047 obNav0411">${items.map(([r,i,l])=>`<button class="${state.route===r?'active':''}" data-route="${r}"><b>${i}</b><span>${l}</span></button>`).join('')}</nav>`;
  }

  function ob411FeatureCard(route,title,body,icon='›',primary=false,prepTab=''){
    const attr=prepTab?` data-prep="${prepTab}"`:` data-route="${route}"`;
    return `<button class="ob411Feature ${primary?'primary':''}"${attr}><b>${esc(title)}</b><small>${esc(body)}</small><em>${esc(icon)}</em></button>`;
  }
  function ob411PlanHeader(title='今できること'){
    const p=current();
    return `<section class="section ob411Header"><div class="head"><div><h2>${esc(title)}</h2><p>主役：${esc(p.title||'未確認箱')} / ${esc(ob411PlanKindLabel(p))}。機能は下バーから選ぶ。</p></div><button class="btn" data-act="planSwitch">切替</button></div></section>`;
  }
  function ob411RecordButtons(){
    return `<div class="ob411QuickGrid">
  ...
  function ob411RecentRecordsHtml(){
    const target=ob411SaveTarget();
  ...
  function ob411RecordHub(){
    const p=current();
    const target=ob411SaveTarget();
    const isWalk=ob411IsWalk();
    const isCamp=ob411IsCamp();
    return shell(`<section class="section obRecord0411">
      <div class="head"><div><h2>${isWalk?'散歩記録':isCamp?'キャンプ記録':'とりあえず記録'}</h2><p>保存先：${esc(target.title)}。迷ったら上から押す。</p></div><span class="pill dark">${esc(target.inbox?'未確認':ob411PlanKindLabel(p))}</span></div>
      ${ob411RecordButtons()}
      <div class="ob411SubGrid">
        ...
        ${isCamp?`<button data-prep="route"><b>ルート</b><small>04.8固定</small></button>`:''}
        <button data-act="openMap"><b>Map</b><small>Google Map</small></button>
        <button data-act="exportGpx"><b>GPX</b><small>履歴出力</small></button>
      </div>
      <div class="honestBox"><b>保存ルール</b><p>${target.inbox?'プラン未選択なので未確認箱へ仮保存。あとで整理する。':'この画面の記録は主役プランに紐付く。毎回確認させない。'}</p></div>
      ${gpsPanelHtml()}
      ${ob411RecentRecordsHtml()}
      ${isCamp?`<details class="ob411Advanced"><summary>詳細現地モード</summary>${ob411LegacyFieldHtml()}</details>`:''}
    </section>`);
  }
  function ob411LegacyFieldHtml(){
    try{return `<div class="ob411Legacy">${fw03HeaderHtml()}${fw03ControlsHtml()}<section class="fw03Middle">${fw03MapHtml()}${fw03PanelHtml()}</section>${fw03FooterHtml()}</div>`}
    catch(e){return `<div class="honestBox"><b>詳細機能</b><p>詳細現地モードは一時的に表示できない。通常記録は上のボタンで使える。</p></div>`}
  }

  home = function(){
    const isCamp=ob411IsCamp(), isWalk=ob411IsWalk();
    return shell(`
      ${ob411PlanHeader('何をする？')}
      <section class="section ob411Home">
        <div class="ob411FeatureGrid">
          ${ob411FeatureCard('calendar','予定を見る','キャンプ/散歩の予定を確認','▦')}
          ${ob411FeatureCard('prep',isWalk?'散歩準備':'準備する',isWalk?'行き先・持ち物・開始へ':'料理・買い物・ギア・天気・ルート','◫',false,'overview')}
          ${ob411FeatureCard('field','記録する','話す / 撮る / 場所 / メモ','＋',true)}
          ${ob411FeatureCard('discover','探す','キャンプ場・散歩場所候補','⌕')}
          ${ob411FeatureCard('memory','思い出','記録一覧・レビュー・次回改善','○')}
        </div>
      </section>
      <section class="section"><div class="honestBox"><b>使い方</b><p>右上は主役プランの確認と切替。使いたい機能は下バーか上の大きいボタンから選ぶ。</p></div></section>
    `);
  }
  function ob411WalkPrepView(){
    const p=current();
    return shell(`<section class="section obWalkPrep0411">
      <div class="head"><div><h2>${esc(p.title||'散歩')}</h2><p>${esc(planSpan(p))} / ${esc(p.place||'場所未設定')}。散歩で必要なものだけ表示。</p></div><span class="pill dark">散歩</span></div>
      <div class="ob411FeatureGrid compact">
        ${ob411FeatureCard('field','散歩記録へ','開始・GPS・写真・音声・メモ','＋',true)}
        <button class="ob411Feature" data-act="toggleWalk"><b>${state.walk.active?'散歩終了':'散歩開始'}</b><small>GPSログを開始/終了</small><em>犬</em></button>
        <button class="ob411Feature" data-act="gps"><b>現在地</b><small>今の場所だけ残す</small><em>⌖</em></button>
        <button class="ob411Feature" data-act="openMap"><b>Map</b><small>Google Mapで見る</small><em>地</em></button>
      </div>
      ${gpsPanelHtml()}
      <div class="honestBox"><b>表示を散歩用に限定</b><p>このプランでは、開始・GPS・写真・音声・メモだけに集中する。</p></div>
    </section>`);
  }
  const ob411BasePrep = prep;
  prep = function(){
    if(ob411IsWalk())return ob411WalkPrepView();
    return ob411BasePrep();
  }
  const ob411BaseRoutePrepView = routePrepView;
  routePrepView = function(){
    if(!ob411IsCamp()){
      return `<section class="section obContext0411"><div class="head"><div><h2>${esc(current().title||'散歩')}</h2><p>この主役プランは${esc(ob411PlanKindLabel())}。記録画面へ案内する。</p></div><span class="pill dark">${esc(ob411PlanKindLabel())}</span></div><div class="honestBox"><b>記録へ進む</b><p>散歩では、開始・GPS・写真・音声・メモを使う。</p><div class="shareOps"><button class="primary" data-route="field">記録へ</button><button data-act="planSwitch">プラン切替</button></div></div></section>`;
    }
    return ob411BaseRoutePrepView();
  }
  field = function(){return ob411RecordHub()}

  const ob411BaseAddMediaRecord = addMediaRecord;
  addMediaRecord = function(kind,file,dataUrl){
    ...
    state.records.push({
      ...
      target:target.inbox?'未確認箱':target.title, flow:[target.title], tags:[ob411PlanKindLabel(), recordKindLabel(kind)].filter(Boolean),
  ...
  addPublicRecord = function(kind,text,title){
    ...
    const p=state.walk.track[state.walk.track.length-1];
    state.records=state.records||[];
    state.records.push({id:uid('rec'),eventId:target.id,planTitle:target.title,kind,title:title||recordKindLabel(kind),text:text||'',mode:target.type==='walk'?'散歩記録':'キャンプ記録',time:new Date().toLocaleTimeString('ja-JP',{hour:'2-digit',minute:'2-digit'}),date:today(),lat:p?.lat||null,lng:p?.lng||null,private:false,fieldModeId:state.fieldMode||'',target:target.inbox?'未確認箱':target.title,flow:[target.title],tags:[ob411PlanKindLabel(),recordKindLabel(kind)].filter(Boolean)});
  }
  const ob411BaseAddFieldRecord = addFieldRecord;
  addFieldRecord = function(kind,text,title,opts={}){
    const target=ob411SaveTarget();
    const p=state.walk.track[state.walk.track.length-1];
    state.records=state.records||[];
    state.records.push({id:uid('rec'),eventId:target.id,planTitle:target.title,kind:kind||'memo',title:title||recordKindLabel(kind)||kind||'記録',text:text||'',mode:target.type==='walk'?'散歩記録':'キャンプ記録',fieldModeId:state.fieldMode||'',target:opts.target||target.title,tags:opts.tags||[ob411PlanKindLabel()],flow:opts.flow||[target.title],time:new Date().toLocaleTimeString('ja-JP',{hour:'2-digit',minute:'2-digit'}),date:today(),lat:p?.lat||null,lng:p?.lng||null,private:!!opts.private});
  }
  sessionRecord = function(data){
    const target=ob411SaveTarget();
    const p=state.walk.track[state.walk.track.length-1];
    const rec={id:uid('rec'),eventId:target.id,planTitle:target.title,kind:data.kind||'memo',title:data.title||data.kind||'記録',text:data.text||'',mode:target.type==='walk'?'散歩記録':'キャンプ記録',sessionGroup:data.group||'session',target:data.target||target.title,flow:data.flow||[target.title],tags:data.tags||[ob411PlanKindLabel()],time:new Date().toLocaleTimeString('ja-JP',{hour:'2-digit',minute:'2-digit'}),date:today(),lat:p?.lat||null,lng:p?.lng||null,private:!!data.private};
    state.records=state.records||[];state.records.push(rec);return rec;
  }


  const ob411BaseCalendar = calendar;
  calendar = function(){
    return ob411BaseCalendar()
      .replace(/<button[^>]*data-act="quickAdd"[^>]*data-type="payment"[\s\S]*?<\/button>/g,'')
      .replace(/支払い確認日/g,'')
      .replace(/カード支払い確認/g,'');
  }
  const ob411BaseQuickAdd = quickAdd;
  quickAdd = function(type){
    if(type==='payment'){toast('支払い予定は今のMVPでは非表示');return;}
    return ob411BaseQuickAdd(type);
  }

  const ob411BaseSelectPlan = selectPlan;
  selectPlan = function(id){
    const e=(state.events||[]).find(x=>x.id===id);
    if(!e)return;
    if(!ob411IsSelectablePlan(e)){toast('主役にできるのはキャンプ/散歩');return;}
    state.currentPlanId=id;
    if(e.start){state.selectedDate=e.start;state.currentMonth=e.start.slice(0,7)}
    state.drawer=null;state.routeFocus='';state.__scrollY=0;
    if(e.type==='walk'){
      if(state.route==='prep')state.prepTab='overview';
      if(state.route==='home')state.route='field';
      state.stage='field';state.walkMode='normal';state.fieldMode='kotaWalk';state.fieldActionMode='kota';
    }else{
      state.stage=state.route==='field'?'field':'prep';
      if(state.route==='prep')state.prepTab=state.prepTab||'overview';
    }
    save();render(false);toast(`${e.type==='walk'?'散歩':'キャンプ'}を主役にした`);
  }
  setCurrentPlan = selectPlan;
  const ob411BaseCreatePaymentTemplate = createPaymentTemplate;
  createPaymentTemplate = function(){toast('支払い予定は今のMVPでは非表示')}

  const ob411BaseBind = bind;
  bind = function(){
    ob411BaseBind();
    document.querySelectorAll('[data-route]').forEach(el=>el.onclick=()=>{
      ...
      state.route=route;
      if(route==='prep'){
        state.prepTab=ob411IsWalk()?'overview':(state.prepTab&&state.prepTab!=='route'?state.prepTab:'overview');
        state.stage=ob411IsWalk()?'field':'prep';
      }else if(route==='field'){
        state.stage='field';
      }
    ...
    document.querySelectorAll('[data-prep]').forEach(el=>el.onclick=()=>{
      state.route='prep';
      state.prepTab=ob411IsWalk()?'overview':(el.dataset.prep||'overview');
      state.stage=ob411IsWalk()?'field':'prep';state.__scrollY=0;save();render(false);
    });
  }
  ...
  */
  ...
  /* RESTORE04.15: 責任者統合版 / UIUXデザイン復旧 + 外出属性モデル最小導入
     - 04.8のルート1画面性を正として復帰
     - 04.13 Context Modelは内部だけ維持
     - 外出タイプは固定メニューで閉じず、属性テンプレートで吸収
     - タブホームは説明ではなく入口
  */
  const OUTBASE_OUTING_ATTRIBUTE_LOCK_0415 = Object.freeze({
    model:'outing_attribute_template',
    rule:'外出タイプを固定メニューで閉じず、宿泊/移動/同伴/予約/天候影響/目的などの属性で必要入口を出し分ける。',
    ui:'画面は予定/探す/準備/記録/思い出の5入口を維持。Contextや属性説明は常時表示しない。',
    route:'ルートはキャンプ専用ではなく外出共通の移動エンジン。ただしキャンプではチェックイン逆算を優先。',
    petEvent:'ペット関連イベントはペット同伴とは限らない。同伴あり/なし/未定で準備入口を変える。'
  });
  Object.assign(typeName,{event:'イベント',daytrip:'日帰り',driveWalk:'ドライブ散歩',lodge:'宿泊',maintenance:'片付け',vehicle:'車準備'});
  function ob415DefaultAttrs(e){
    const type=e?.type||'normal';
    const title=e?.title||'';
    const isCamp=type==='camp';
    const isWalk=type==='walk';
    const isEvent=type==='event'||/イベント|フェス|雪峰祭|展示|マルシェ/.test(title);
    const isDriveWalk=type==='driveWalk'||/ドライブ散歩|公園|湖|道の駅/.test(title+String(e?.memo||''));
    const isSearch=type==='search';
    return {
      template:isCamp?'camp':isEvent?'event':isDriveWalk?'driveWalk':isWalk?'homeWalk':isSearch?'search':'daytrip',
      stay:isCamp?'overnight':'none',
      move:isWalk&&!isDriveWalk?'walk':'car',
      petCompanion:isCamp||isWalk||isDriveWalk?'yes':isEvent?'unknown':'unknown',
      companionPets:isCamp||isWalk||isDriveWalk?['kota']:[],
      companions:['mu','rin'],
      reservation:isCamp?'yes':isEvent?'unknown':'unknown',
      ticket:isEvent?'unknown':'none',
      weatherImpact:isCamp||isWalk||isDriveWalk?'high':'medium',
      purposes:isCamp?['camp','record','review']:isWalk?['walk','health','record']:isDriveWalk?['drive','walk','scout','record']:isEvent?['event','shopping','research','record']:isSearch?['scout','candidate']:['outing','record'],
      routeDeadlineLabel:isCamp?'チェックイン':isEvent?'入場/開始':isWalk&&!isDriveWalk?'散歩開始':'到着',
      routeDeadlineTime:isCamp?'13:00':isEvent?'10:00':'10:30',
      updatedAt:ob413Now?.()||new Date().toISOString()
    };
  }
  function ob415EnsureOutingModel(){
    ob413NormalizeContext?.();
    state.outingAttributes=state.outingAttributes&&typeof state.outingAttributes==='object'?state.outingAttributes:{};
    (state.events||[]).forEach(e=>{state.outingAttributes[e.id]={...ob415DefaultAttrs(e),...(state.outingAttributes[e.id]||{})};});
    state.outingTemplates=state.outingTemplates||[
      {id:'homeWalk',label:'通常散歩',attrs:{template:'homeWalk',move:'walk',petCompanion:'yes',companionPets:['kota'],purposes:['walk','health','record']}},
      {id:'driveWalk',label:'ドライブ散歩',attrs:{template:'driveWalk',move:'car',petCompanion:'yes',companionPets:['kota'],purposes:['drive','walk','scout','record']}},
      {id:'camp',label:'キャンプ',attrs:{template:'camp',stay:'overnight',move:'car',petCompanion:'yes',companionPets:['kota'],purposes:['camp','food','gear','record','review']}},
      {id:'eventWithPet',label:'イベント 同伴あり',attrs:{template:'event',move:'car',petCompanion:'yes',companionPets:['kota'],ticket:'unknown',purposes:['event','shopping','record']}},
      {id:'eventNoPet',label:'イベント 同伴なし',attrs:{template:'event',move:'car',petCompanion:'no',companionPets:[],ticket:'unknown',purposes:['event','shopping','research','record']}},
      {id:'inbox',label:'予定なし記録',attrs:{template:'inbox',move:'unknown',petCompanion:'unknown',purposes:['quickRecord','laterLink']}}
    ];
    if(!(state.events||[]).some(e=>e.id==='drivewalk_sample')){
      state.events.push({id:'drivewalk_sample',title:'手賀沼ドライブ散歩',type:'driveWalk',start:'2026-07-13',end:'2026-07-13',repeat:'none',place:'手賀沼周辺',memo:'車で行ってからコタ散歩。駐車場と日陰を確認。',level:2});
      state.outingAttributes.drivewalk_sample={...ob415DefaultAttrs({type:'driveWalk',title:'手賀沼ドライブ散歩'}),template:'driveWalk',routeDeadlineTime:'10:30'};
    }
    if(!(state.events||[]).some(e=>e.id==='petevent_sample')){
      state.events.push({id:'petevent_sample',title:'ペット関連イベント 下見',type:'event',start:'2026-07-26',end:'2026-07-26',repeat:'none',place:'イベント会場',memo:'今回は人だけで物販と情報収集。次回コタ同伴可否を見る。',level:2});
      state.outingAttributes.petevent_sample={...ob415DefaultAttrs({type:'event',title:'ペット関連イベント 下見'}),petCompanion:'no',companionPets:[],purposes:['event','shopping','research','scout'],routeDeadlineTime:'10:00'};
    }
    state.outingAudit0415={version:VERSION,time:ob413Now?.()||new Date().toISOString(),events:(state.events||[]).length,templates:(state.outingTemplates||[]).length};
  }
  function ob415EventForTarget(t){
    if(!t)return current();
    if(t.type==='session')return ob413Event(t.rootPlanId)||current();
    if(t.type==='plan')return ob413Event(t.id)||current();
    return current();
  }
  function ob415ViewTarget(){try{return ob413ViewTarget()}catch(e){return current()}}
  function ob415RecordTarget(){try{return ob413RecordTarget()}catch(e){return current()}}
  function ob415ViewEvent(){return ob415EventForTarget(ob415ViewTarget())}
  function ob415Attrs(e=ob415ViewEvent()){ob415EnsureOutingModel();return {...ob415DefaultAttrs(e),...(state.outingAttributes?.[e?.id]||{})}}
  function ob415PetLabel(attrs){
    if(attrs.petCompanion==='yes')return `同伴 ${attrs.companionPets?.length?attrs.companionPets.map(id=>(state.pets||[]).find(p=>p.id===id)?.name||id).join('・'):'あり'}`;
    if(attrs.petCompanion==='no')return '同伴なし';
    return '同伴未定';
  }
  function ob415SmallStatus(){
    ...
    return `<button class="ob415Status ${diff?'split':''}" data-act="planSwitch"><span><b>表示</b>${esc(ob413TargetLabel?.(v)||v?.title||'未確認')}</span><span><b>保存</b>${esc(ob413TargetLabel?.(r)||r?.title||'未確認')}</span><em>${esc(ob415PetLabel(a))}</em></button>`;
  }
  function ob415Shell(inner){return shell(`${ob415SmallStatus()}${inner}`)}
  function ob415Hero(title,sub,kicker='OUTBASE'){
    return `<section class="ob415Hero"><div><span>${esc(kicker)}</span><h1>${esc(title)}</h1><p>${esc(sub)}</p></div><button data-act="planSwitch">切替</button></section>`;
  }
  function ob415CardGrid(items,cls=''){
    return `<div class="ob415Grid ${cls}">${items.map(i=>`<button class="${i.primary?'primary':''}" ${i.route?`data-route="${i.route}"`:''} ${i.prep?`data-prep="${i.prep}"`:''} ${i.act?`data-act="${i.act}"`:''} ${i.id?`data-id="${i.id}"`:''}><b>${esc(i.title)}</b><small>${esc(i.sub||'')}</small></button>`).join('')}</div>`;
  }
  function ob415PrepItems(){
    const e=ob415ViewEvent(), a=ob415Attrs(e);
    const items=[{title:'ルート',sub:a.template==='camp'?'出発逆算・寄り道・Maps':'目的地・駐車場・到着時間',prep:'route',primary:true}];
    if(['camp','daytrip','event','driveWalk'].includes(a.template)||a.move==='car')items.push({title:'持ち物',sub:ob415PetLabel(a)+' / 車載・水・休憩',prep:'gear'});
    if(a.template==='camp')items.push({title:'買い物',sub:'食材・消耗品・未購入',prep:'shopping'},{title:'料理',sub:'献立・材料・手順',prep:'meals'},{title:'ギア',sub:'積込・収納・乾燥',prep:'gear'});
    if(a.template==='event'){
      items.push({title:'入場/ブース',sub:a.petCompanion==='no'?'見たいブース・買いたい物':'入場・休憩・混雑回避',act:'eventPrepNote'});
      items.push({title:'買い物',sub:'物販・気になる商品',prep:'shopping'});
    }
    if(a.template==='driveWalk'||a.template==='homeWalk')items.push({title:'散歩',sub:a.move==='car'?'現地散歩・駐車場・日陰':'自宅からそのまま記録',act:a.move==='car'?'startDriveWalkSession':'startNormalWalkSession'});
    items.push({title:'天気',sub:a.weatherImpact==='high'?'雨風・暑さ寒さを優先':'当日確認',prep:'weather'});
    return items;
  }
  function ob415PlanLine(e=ob415ViewEvent()){
    const a=ob415Attrs(e), date=e?.start?`${fmt(e.start)}${e.end&&e.end!==e.start?`〜${fmt(e.end)}`:''}`:'日付未設定';
    return `${typeName[e?.type]||a.template} / ${date} / ${ob415PetLabel(a)}`;
  }
  // Top/Nav: 04.8の落ち着いた見た目へ戻す。Context説明は出さない。
  top = function(){
    const e=ob415ViewEvent(), r=ob415RecordTarget();
    return `<div class="top obTop046 obTop0415"><div class="top-row"><button class="brand" data-route="home"><span class="logo">OB</span><span><b>OUTBASE</b><small>field system</small></span></button><button class="plan" data-act="planSwitch"><i></i><b>${esc(e?.title||'未確認')}</b><small>${esc(ob415PlanLine(e))}</small></button></div><button class="ob415SaveStrip" data-act="planSwitch"><b>保存先：${esc(ob413TargetLabel?.(r)||r?.title||'未確認')}</b><small>表示切替では保存先を変えない</small></button></div>`;
  };
  nav = function(){
    const items=[['calendar','▦','予定'],['discover','⌕','探す'],['prep','◫','準備'],['field','＋','記録'],['memory','○','思い出']];
    return `<nav class="bottomNav obNav047 obNav0415">${items.map(([r,i,l])=>`<button class="${state.route===r?'active':''}" data-route="${r}"><b>${i}</b><span>${l}</span></button>`).join('')}</nav>`;
  };
  // Route: 04.8の一画面構成を維持しつつ、キャンプ以外の外出にも同期。
  function ob415SyncRouteToView(){
    const e=ob415ViewEvent(); if(!e)return;
    const a=ob415Attrs(e);
    state.routePlan=state.routePlan||{};
    if(state.routePlan.eventId!==e.id){
      state.routePlan.eventId=e.id;
      state.routePlan.destination=e.place||state.routePlan.destination||'';
      state.routePlan.checkin=a.routeDeadlineTime||state.routePlan.checkin||'13:00';
      state.routePlan.arrivalTarget=state.routePlan.arrivalTarget||'b30';
      state.routePlan.driveMin=Number(state.routePlan.driveMin||150);
      state.routePlan.trafficBuffer=Number(state.routePlan.trafficBuffer||20);
    }
  }
  function ob415RoutePlanLine(){
    const e=ob415ViewEvent(), a=ob415Attrs(e); const d=e?.start?`${fmt(e.start)}${e.end&&e.end!==e.start?`〜${fmt(e.end)}`:''}`:'';
    const label=a.routeDeadlineLabel||'到着';
    return `${esc(e?.title||routeTarget())}${d?` / ${esc(d)}`:''} / ${esc(label)} ${esc(state.routePlan?.checkin||a.routeDeadlineTime||'13:00')}`;
  }
  const ob415BaseOpenSimpleRoute = typeof openSimpleRoute==='function'?openSimpleRoute:null;
  const ob415BaseRouteTarget = routeTarget;
  routeTarget = function(){
    const e=ob415ViewEvent();
    return state.routePlan?.destination || e?.place || ob415BaseRouteTarget?.() || '';
  };
  const ob415BaseRoutePrepView = routePrepView;
  routePrepView = function(){
    ob415EnsureOutingModel(); ob415SyncRouteToView();
    const at=state.routePlan?.arrivalTarget||'b30';
    const current=ob048DepartForTarget(at,'採用');
    const roomy=ob048DepartForTarget('b60','余裕あり');
    const standard=ob048DepartForTarget('b30','標準');
    const tight=ob048DepartForTarget('exact','ギリギリ');
    const rows=simpleRouteRows();
    return `<section class="obRouteDash obRoute045 obRoute048 obRoute0415">
      <div class="obHeroCompact">
        <div class="obHeroCompactMain"><span class="label">自宅出発</span><strong>${current.depart}</strong><em>${routeArrivalLabel()}</em></div>
        <p>${ob415RoutePlanLine()}</p>
        <div class="obHeroMiniStats"><span>移動<b>${Number(state.routePlan?.driveMin||150)}分</b></span><span>寄り道<b>${simpleStopTotal()}分</b></span><span>予備<b>${Number(state.routePlan?.trafficBuffer||0)}分</b></span></div>
        <div class="obHeroCompactActions"><button class="primary" data-act="saveSimpleRoute">保存</button><button data-act="openSimpleRoute">Maps</button></div>
      </div>
      ${ob045SavedPlan()}${ob045PlanSelect()}
      <div class="obStopMiniGrid">${ob045StopButtons()}</div>
      <div class="obDepartMiniBand"><button class="${at==='b60'?'active':''}" data-act="setArrivalTarget" data-target="b60"><span>余裕あり</span><b>${roomy.depart}</b></button><button class="${at==='b30'||at==='b15'?'active':''}" data-act="setArrivalTarget" data-target="b30"><span>標準</span><b>${standard.depart}</b></button><button class="${at==='exact'?'active':''}" data-act="setArrivalTarget" data-target="exact"><span>ギリギリ</span><b>${tight.depart}</b></button></div>
      <details class="obEdit obEditCompact"><summary><b>調整</b><small>移動時間・到着目標・自宅</small></summary><div class="obEditGrid">${ob045HomeEdit()}<label class="obMiniField"><span>${esc(ob415Attrs().routeDeadlineLabel||'到着')}</span><input data-route-field="checkin" type="time" value="${esc(state.routePlan?.checkin||'13:00')}"></label><label class="obMiniField"><span>当日想定移動</span><input data-route-field="driveMin" type="number" value="${Number(state.routePlan?.driveMin||150)}"></label><label class="obMiniField"><span>到着目標</span><select data-route-field="arrivalTarget"><option value="exact" ${state.routePlan?.arrivalTarget==='exact'?'selected':''}>ちょうど</option><option value="b15" ${state.routePlan?.arrivalTarget==='b15'?'selected':''}>15分前</option><option value="b30" ${(state.routePlan?.arrivalTarget||'b30')==='b30'?'selected':''}>30分前</option><option value="b60" ${state.routePlan?.arrivalTarget==='b60'?'selected':''}>1時間前</option><option value="late" ${state.routePlan?.arrivalTarget==='late'?'selected':''}>少し遅れてOK</option></select></label><label class="obMiniField"><span>追加予備</span><input data-route-field="trafficBuffer" type="number" value="${Number(state.routePlan?.trafficBuffer||20)}"></label></div><div class="obMinuteGrid">${ob041StopMinuteInputs()}</div></details>
      <details class="obEdit obEditCompact"><summary><b>内訳</b><small>出発から到着目標まで</small></summary><div class="obTimeline">${rows.map(r=>`<div><time>${esc(r.time)}</time><span><b>${esc(r.label)}</b><small>${esc(r.memo)}</small></span></div>`).join('')}</div></details>
    </section>`;
  };
  prep = function(){
    if(state.prepTab==='route')return ob415Shell(`${routePrepView()}`);
    ...
    const e=ob415ViewEvent();
    return ob415Shell(`${ob415Hero('準備',`${e?.title||'外出'}に必要な入口だけ表示。細かい説明は中に入れてから見る。`,'READY')}${ob415CardGrid(ob415PrepItems())}`);
  };
  field = function(){
    const r=ob415RecordTarget();
    return ob415Shell(`<section class="ob415RecordHero"><div><span>保存先</span><h1>${esc(ob413TargetLabel?.(r)||r?.title||'未確認')}</h1><p>話す・撮る・場所・ピン・メモ。予定なしでも未確認箱へ逃がす。</p></div></section>${ob415CardGrid([
      {title:voiceRecorder?'停止':'話す',sub:'音声メモ',act:'toggleVoice',primary:true},{title:'撮る',sub:'写真',act:'capturePhoto'},{title:'動画',sub:'必要時だけ',act:'captureVideo'},{title:'場所',sub:'現在地を保存',act:'gps'},{title:'ピン',sub:'地点メモ',act:'addPinQuick'},{title:'メモ',sub:'一言記録',act:'quickRecord'}
    ],'record')}${ob415CardGrid([
      {title:state.walk?.active?'散歩終了':'散歩開始',sub:'GPSログ',act:'toggleWalk',primary:true},{title:'通常散歩',sub:'自宅から歩く',act:'startNormalWalkSession'},{title:'ドライブ散歩',sub:'車で行ってから歩く',act:'startDriveWalkSession'},{title:'場内散歩',sub:'キャンプ/イベント先',act:'startCampWalkSession'}
    ],'mini')}${gpsPanelHtml()}${ob412RecentRecordsHtml?ob412RecentRecordsHtml():''}`);
  };
  home = function(){
    const e=ob415ViewEvent(), a=ob415Attrs(e);
    return ob415Shell(`${ob415Hero('何をする？',`${e?.title||'外出'} / ${ob415PetLabel(a)}。迷ったら大きい入口だけ押す。`,'FIELD OS')}${ob415CardGrid([
      {title:'準備する',sub:'ルート・持ち物・買い物・天気',route:'prep',primary:true},{title:'記録する',sub:'話す・撮る・場所・ピン',route:'field'},{title:'予定を見る',sub:'今日・次回・切替',route:'calendar'},{title:'探す',sub:'候補・下見・保存',route:'discover'},{title:'思い出',sub:'整理・レビュー・次回改善',route:'memory'}
    ])}${ob415CardGrid([
      {title:'ルートを開く',sub:'04.8の一画面ルート',prep:'route',primary:true},{title:'通常散歩',sub:'自宅からすぐ記録',act:'startNormalWalkSession'},{title:'ドライブ散歩',sub:'車移動＋現地散歩',act:'startDriveWalkSession'},{title:'予定なし記録',sub:'あとで整理',act:'recordInbox'}
    ],'mini')}`);
  };
  const ob415BaseCalendar = calendar;
  calendar = function(){
    ...
    return ob415Shell(`${ob415Hero('予定','外出プランを選ぶ。ペット同伴は属性で持つので、イベント=同伴とは決めない。','PLAN')}${ob415CardGrid([
      {title:'キャンプ',sub:'泊まり/デイキャンプにも展開',act:'createCampTemplate',primary:true},{title:'通常散歩',sub:'自宅から歩く',act:'createHomeWalkTemplate'},{title:'ドライブ散歩',sub:'車で行ってから散歩',act:'createDriveWalkTemplate'},{title:'イベント 同伴あり',sub:'コタ連れ想定',act:'createEventWithPetTemplate'},{title:'イベント 同伴なし',sub:'人だけで下見/物販',act:'createEventNoPetTemplate'},{title:'予定なし記録',sub:'未確認箱へ',act:'recordInbox'}
    ],'mini')}<section class="section ob415List"><div class="head"><div><h2>外出予定</h2><p>表示だけ変えても保存先は変わらない。</p></div></div><div class="list">${selectable.map(e=>`<button class="row" data-act="viewPlan" data-id="${e.id}"><span><strong>${esc(e.title)}</strong><small>${esc(ob415PlanLine(e))} / ${esc(e.place||'場所未設定')}</small></span><em>表示</em></button>`).join('')}</div></section>`);
  };
  discover = function(){
    return ob415Shell(`${ob415Hero('探す','候補探し・下見・保存。準備や記録と混ぜない。','DISCOVER')}${ob415CardGrid([
      {title:'キャンプ場',sub:'犬可・温水・4時間以内',act:'findCampCandidate',primary:true},{title:'散歩場所',sub:'駐車場・日陰・水辺',act:'findWalkCandidate'},{title:'ペットイベント',sub:'同伴あり/なしを後で設定',act:'createEventNoPetTemplate'},{title:'ドッグカフェ/道の駅',sub:'寄り道候補',act:'findWalkCandidate'},{title:'下見記録',sub:'駐車場・トイレ・混雑',act:'quickRecord'},{title:'保存候補',sub:'思い出側で整理',route:'memory'}
    ])}`);
  };
  memory = function(){
    return ob415Shell(`${ob415Hero('思い出','帰宅後の整理・レビュー・次回改善。現地記録とは分ける。','MEMORY')}${ob415CardGrid([
      {title:'記録一覧',sub:'写真・音声・メモ',act:'showLegacyMemory',primary:true},{title:'次回改善',sub:'買い足し/撤退理由/反省',act:'addReview'},{title:'場所メモ',sub:'駐車場・トイレ・日陰',act:'recordToPlace'},{title:'最後の記録を表示中へ関連',sub:'複数リンク',act:'linkLastToView'}
    ])}<details class="ob415Details"><summary>既存の思い出詳細を開く</summary>${(typeof publicRecords==='function')?publicRecords().map(fieldRecordCard).join(''):''}</details>`);
  };
  const ob415BaseAct = act;
  act = function(a,el){
    ob415EnsureOutingModel();
    if(a==='capturePhoto')return act('captureMedia',{dataset:{kind:'photo'}});
    if(a==='captureVideo')return act('captureMedia',{dataset:{kind:'video'}});
    if(a==='startDriveWalkSession'){
      let e=(state.events||[]).find(x=>x.id==='drivewalk_sample');
      if(!e){e={id:'drivewalk_sample',title:'ドライブ散歩',type:'driveWalk',start:today(),end:today(),repeat:'none',place:'目的地未設定',memo:'車で行ってから散歩',level:2};state.events.push(e)}
      state.outingAttributes[e.id]={...ob415DefaultAttrs(e),template:'driveWalk',move:'car',petCompanion:'yes',companionPets:['kota']};
      ob413SetView(e.id,'plan',{render:false,toast:false});
      const s=ob413StartSession('walk',e.id,'現地散歩',{render:false,toast:false});
      state.route='field'; save(); render(false); toast('ドライブ散歩を記録先にした'); return;
    }
    if(a==='createHomeWalkTemplate')return ob415CreatePlan('walk','コタ通常散歩','自宅周辺',{template:'homeWalk',move:'walk',petCompanion:'yes',companionPets:['kota']});
    if(a==='createDriveWalkTemplate')return ob415CreatePlan('driveWalk','ドライブ散歩','目的地未設定',{template:'driveWalk',move:'car',petCompanion:'yes',companionPets:['kota']});
    if(a==='createCampTemplate')return ob415CreatePlan('camp','新しいキャンプ','場所未設定',{template:'camp',stay:'overnight',move:'car',petCompanion:'yes',companionPets:['kota']});
    if(a==='createEventWithPetTemplate')return ob415CreatePlan('event','ペット関連イベント 同伴あり','会場未設定',{template:'event',move:'car',petCompanion:'yes',companionPets:['kota'],ticket:'unknown'});
    if(a==='createEventNoPetTemplate')return ob415CreatePlan('event','ペット関連イベント 同伴なし','会場未設定',{template:'event',move:'car',petCompanion:'no',companionPets:[],ticket:'unknown',purposes:['event','shopping','research']});
    if(a==='eventPrepNote'){toast('イベント準備は見たいブース/買い物/入場時間を記録');return;}
    if(a==='findCampCandidate'||a==='findWalkCandidate'){toast('候補は写真・メモ・場所で記録して保存');return;}
    if(a==='showLegacyMemory'){toast('下の詳細を開いて確認');return;}
    return ob415BaseAct(a,el);
  };
  function ob415CreatePlan(type,title,place,attrs){
    const id=uid('plan');
    const e={id,title,type,start:today(),end:today(),repeat:'none',place,memo:'OUTBASE 04.15属性テンプレートから作成',level:1};
    state.events.push(e); state.outingAttributes[id]={...ob415DefaultAttrs(e),...attrs};
    ob413SetView(id,'plan',{render:false,toast:false}); ob413SetRecord(id,'plan',{render:false,toast:false}); state.route='calendar'; save(); render(false); toast(`${title}を作成`);
  }
  // 04.12/04.13の大きな文脈説明は常時表示しない。必要情報は小型ステータスだけにする。
  ob412DuoStatusHtml = function(){return ob415SmallStatus()};
  ob412ConcurrentPanel = function(){return ''};
  ob412InjectConcurrent = function(html){return html};

  function ob415FinalAudit(){
    ob415EnsureOutingModel();
    const checks=[
      ['context model kept',!!state.context&&Array.isArray(state.sessions)&&Array.isArray(state.links)],
      ['outing attrs exists',!!state.outingAttributes&&!!state.outingTemplates],
      ['route 04.8 functions kept',typeof ob048DepartForTarget==='function'&&typeof simpleRouteRows==='function'],
      ['tab homes are entry functions',typeof home==='function'&&typeof prep==='function'&&typeof field==='function'],
      ['payment not normal route',!(state.events||[]).some(e=>e.type==='payment')]
    ];
  ...
  /* RESTORE04.17: トップ=カレンダー / 現地=散歩地図。外出タイプ選択は新規追加の中へ下げる。 */
  function ob417EventsOn(date){
    return (state.events||[]).filter(e=>{
      if(!e.start)return false;
      const st=e.start, en=e.end||e.start;
      return st<=date && date<=en;
    });
  }
  function ob417SortedEvents(list){
    return [...(list||[])].sort((a,b)=>(a.start||'9999').localeCompare(b.start||'9999') || (a.title||'').localeCompare(b.title||''));
  }
  function ob417NextEvents(limit=4){
    const t=today();
    return ob417SortedEvents((state.events||[]).filter(e=>e.start && (e.end||e.start)>=t && e.type!=='payment')).slice(0,limit);
  }
  function ob417TodayEvents(){return ob417SortedEvents(ob417EventsOn(today()).filter(e=>e.type!=='payment'))}
  function ob417MonthMatrix(){
    const month=state.currentMonth||today().slice(0,7);
    const [y,m]=month.split('-').map(Number);
    const first=new Date(y,m-1,1);
    const start=new Date(first); start.setDate(1-first.getDay());
    return Array.from({length:42},(_,i)=>{const d=new Date(start);d.setDate(start.getDate()+i);return iso(d)});
  }
  function ob417DayCell(d){
    const month=state.currentMonth||today().slice(0,7);
    const ev=ob417EventsOn(d).filter(e=>e.type!=='payment');
    const cls=[d.slice(0,7)!==month?'out':'', d===today()?'today':'', d===state.selectedDate?'selected':''].filter(Boolean).join(' ');
    return `<button class="ob417Day ${cls}" data-act="selectDay" data-date="${esc(d)}"><b>${Number(d.slice(8,10))}</b>${ev.slice(0,2).map(e=>`<span class="${e.type==='camp'?'camp':''}">${esc((e.title||'予定').slice(0,7))}</span>`).join('')}${ev.length>2?`<em>+${ev.length-2}</em>`:''}</button>`;
  }
  function ob417CalendarBoard(){
    const month=state.currentMonth||today().slice(0,7);
    const [y,m]=month.split('-').map(Number);
    const days=ob417MonthMatrix();
    return `<section class="ob417CalendarFirst"><div class="ob417CalHead"><button class="calBtn" data-act="prevMonth">‹</button><div><b>${y}.${pad(m)}</b><small>予定を見る場所。外出タイプ選択は＋新規予定の中。</small></div><button class="calBtn" data-act="nextMonth">›</button></div><div class="ob417Week">${['日','月','火','水','木','金','土'].map(x=>`<span>${x}</span>`).join('')}</div><div id="days" class="ob417Days">${days.map(ob417DayCell).join('')}</div><div class="ob417CalOps"><button data-act="goToday">今日</button><button class="primary" data-act="addEvent" data-date="${esc(state.selectedDate||today())}">＋新規予定</button></div></section>`;
  }
  function ob417EventRow(e,action='viewPlan'){
    const a=ob415Attrs(e);
    return `<button class="ob417EventRow" data-act="${action}" data-id="${esc(e.id)}"><span><b>${esc(e.title||'予定')}</b><small>${esc(ob415PlanLine(e))} / ${esc(e.place||'場所未設定')}</small></span><em>${esc(ob415PetLabel(a))}</em></button>`;
  }
  function ob417SelectedPanel(){
    const date=state.selectedDate||today();
    const list=ob417SortedEvents(ob417EventsOn(date).filter(e=>e.type!=='payment'));
    return ob416Panel(`${fmt(date)} の予定`,list.length?`${list.length}件`:'空き',list.length?list.map(e=>ob417EventRow(e)).join(''):`<div class="ob416Empty"><b>予定なし</b><small>＋新規予定から追加。記録だけなら記録タブで未確認箱へ。</small></div>`);
  }
  function ob417TodayPanel(){
    const list=ob417TodayEvents();
    return ob416Panel('今日',list.length?`${list.length}件`:'今日の予定なし',list.length?list.map(e=>ob417EventRow(e)).join(''):`<div class="ob416Empty"><b>今日の予定なし</b><small>散歩や記録は予定なしでも開始できる。</small></div>`);
  }
  function ob417NextPanel(){
    const list=ob417NextEvents(4);
    return ob416Panel('次の外出',list.length?'近い順':'まだなし',list.length?list.map(e=>ob417EventRow(e)).join(''):`<div class="ob416Empty"><b>次の外出なし</b><small>＋新規予定で作成。</small></div>`);
  }
  function ob417NewPlanDock(){
    return `<details class="ob417NewDock"><summary>＋ 新規予定の種類を選ぶ</summary>${ob416ActionGrid([
      {title:'キャンプ',sub:'泊まり・デイキャンプ',act:'createCampTemplate',primary:true},
      {title:'通常散歩',sub:'自宅から歩く',act:'createHomeWalkTemplate'},
      {title:'ドライブ散歩',sub:'車で行ってから歩く',act:'createDriveWalkTemplate'},
      {title:'同伴イベント',sub:'ペット連れ',act:'createEventWithPetTemplate'},
      {title:'人だけイベント',sub:'物販・下見',act:'createEventNoPetTemplate'},
      {title:'予定なし記録',sub:'未確認箱へ',act:'recordInbox'}
    ],'mini')}</details>`;
  }
  function ob417WalkMapPanel(){
    const km=walkDistance(), sec=walkDurationSec(), sp=walkAvgSpeed();
    const pts=(state.walk.track||[]).length;
    const pins=(state.walk.spots||[]).length+(state.mapPins||[]).length;
    return `<section class="ob417WalkMap"><div class="ob417MapHead"><span><b>散歩マップ</b><small>移動ルートとは別。歩いた軌跡・現在地・ピン・写真場所を残す。</small></span><span class="gpsLive ${state.walk.active?'on':''}">${state.walk.active?'LIVE':'MAP'}</span></div><div class="ob417MapCanvas"><canvas id="walkMap" width="680" height="360"></canvas>${pts?`<div class="ob417MapBadge">GPS ${pts}点</div>`:`<div class="ob417MapEmpty">散歩開始 / 現在地で地図に記録</div>`}</div><div class="ob417MapStats"><span><b>${km.toFixed(2)}</b><small>km</small></span><span><b>${fmtDuration(sec)}</b><small>時間</small></span><span><b>${sp.toFixed(1)}</b><small>km/h</small></span><span><b>${pins}</b><small>ピン</small></span></div><div class="ob417MapOps"><button class="primary" data-act="toggleWalk">${state.walk.active?'散歩終了':'散歩開始'}</button><button data-act="gps">現在地</button><button data-act="addPinQuick">ピン</button><button data-act="openMap">Google Map</button></div></section>`;
  }
  function ob417RecordTools(){
    return [
      {title:voiceRecorder?'停止':'話す',sub:voiceRecorder?'録音中':'音声メモ',act:'toggleVoice',primary:true},
      {title:'撮る',sub:'写真',act:'capturePhoto'},
      {title:'メモ',sub:'一言記録',act:'quickRecord'},
      {title:'場所',sub:'現在地保存',act:'gps'},
      {title:'ピン',sub:'地図に残す',act:'addPinQuick'},
      {title:'動画',sub:'必要時だけ',act:'captureVideo'}
    ];
  }
  calendar = function(){
    ob415EnsureOutingModel();
    return ob416Shell(`${ob417CalendarBoard()}${ob417TodayPanel()}${ob417NextPanel()}${ob417SelectedPanel()}${ob417NewPlanDock()}`);
  };
  home = function(){
    return calendar();
  };
  field = function(){
    const r=ob415RecordTarget();
    return ob416Shell(`${ob416Hero('記録',`保存先：${ob413TargetLabel?.(r)||r?.title||'未確認箱'}。地図は移動ルートではなく現地・散歩記録。`,'RECORD','保存')}${ob417WalkMapPanel()}${ob416ActionGrid(ob417RecordTools(),'record')}${ob416Panel('散歩セッション','自宅散歩・ドライブ散歩・場内/会場散歩を分ける',ob416ActionGrid([
      {title:'通常散歩',sub:'自宅から歩く',act:'startNormalWalkSession',primary:true},
      {title:'ドライブ散歩',sub:'車で行ってから歩く',act:'startDriveWalkSession'},
      {title:'場内/会場散歩',sub:'キャンプ・イベント先',act:'startCampWalkSession'},
      {title:'未確認箱',sub:'予定なし記録',act:'recordInbox'}
  ...
  prep = function(){
    ...
    if(state.prepTab&&state.prepTab!=='overview')return shell(`<div class="ob416Page">${ob416Status()}<section class="section ob416LegacyTabs"><div class="tabs">${['overview:全体','route:ルート','meals:料理','shopping:買い物','gear:ギア','weather:天気','pets:ペット','timer:タイマー'].map(x=>{const [id,l]=x.split(':');return `<button class="tab ${state.prepTab===id?'active':''}" data-prep="${id}">${l}</button>`}).join('')}</div></section>${prepBody()}</div>`);
    const e=ob415ViewEvent(), a=ob415Attrs(e);
    return ob416Shell(`${ob416Hero('準備',`${e?.title||'外出'}に必要な入口だけ。ルートは移動、散歩地図は記録タブ。`,'READY',a.move==='car'?'車移動':'準備')}${ob416MiniBar([{label:'目的地',value:e?.place||'場所未設定',active:true},{label:'同伴',value:ob415PetLabel(a)},{label:'天気',value:a.weatherImpact==='high'?'要注意':'確認'}])}${ob416ActionGrid(ob415PrepItems().map((x,i)=>({...x,primary:i===0})))}`);
  };
  const ob417BaseAct = act;
  act = function(a,el){
    if(a==='selectDay'){
      state.selectedDate=el.dataset.date||today();
      state.currentMonth=state.selectedDate.slice(0,7);
      save();render();return;
    }
    return ob417BaseAct(a,el);
  };
  function ob417FinalAudit(){
    ob415EnsureOutingModel();
    const checks=[
      ['calendar is first surface',/ob417CalendarFirst/.test(calendar()) && !/^<section[^>]*>.*キャンプ.*通常散歩/s.test(calendar().slice(0,1000))],
      ['new outing types are behind details',/ob417NewDock/.test(calendar())],
      ['walk map visible in record',/walkMap/.test(field()) && /散歩マップ/.test(field())],
      ['route remains available',typeof routePrepView==='function' && /自宅出発/.test(routePrepView())],
      ['context and outing attrs kept',!!state.context && !!state.outingAttributes]
    ];
    state.lastAudit0417={version:VERSION,time:ob413Now?.()||new Date().toISOString(),checks:checks.map(([name,pass])=>({name,pass:!!pass})),pass:checks.every(x=>x[1]),decision:checks.every(x=>x[1])?'zip_candidate':'stop'};
  }
  ob417FinalAudit();


  /* RESTORE04.18: ルート以外の全体UIUX再統合版
     固定:
       - ルート04.8系の一画面性は触らない
       - トップはカレンダー主役
       - 外出タイプは＋新規予定の中
       - 土日祝・連泊/連動日程バー・日付タップ動作・スクロール保持
       - 記録タブに散歩マップ
  */
  ...
  function ob418HolidayName(date){return ob418HolidayMap()[date]||''}
  function ob418Dow(date){return dObj(date)?.getDay?.()??0}
  function ob418DayMark(date){
    const dow=ob418Dow(date), hol=ob418HolidayName(date);
    if(hol)return {kind:'holiday',label:'祝',name:hol};
    if(dow===0)return {kind:'sun',label:'日',name:'日曜'};
    if(dow===6)return {kind:'sat',label:'土',name:'土曜'};
    return {kind:'weekday',label:['日','月','火','水','木','金','土'][dow]||'',name:''};
  }
  function ob418MonthDays(){
    const month=state.currentMonth||today().slice(0,7);
    const [y,m]=month.split('-').map(Number);
    const first=new Date(y,m-1,1);
  ...
  function ob418EventsOn(date){
    return ob417SortedEvents((state.events||[]).filter(e=>{
      if(!e.start || e.type==='payment')return false;
      const st=e.start, en=e.end||e.start;
      return st<=date && date<=en;
    }));
  }
  function ob418MultiEventsOn(date){return ob418EventsOn(date).filter(e=>(e.end||e.start)!==e.start)}
  function ob418SingleEventsOn(date){return ob418EventsOn(date).filter(e=>(e.end||e.start)===e.start)}
  function ob418EventPhase(e,date){
    const st=e.start, en=e.end||e.start;
    if(st===en)return '単日';
    if(date===st)return '初日';
    if(date===en)return '最終日';
    return '滞在日';
  }
  function ob418BarLabel(e,date){return date===e.start?esc((e.title||'予定').slice(0,9)):''}
  function ob418DayCell(date){
    const month=state.currentMonth||today().slice(0,7);
    const mark=ob418DayMark(date);
    const multis=ob418MultiEventsOn(date).slice(0,2);
    const singles=ob418SingleEventsOn(date).slice(0,2);
    const cls=['ob418Day',date.slice(0,7)!==month?'out':'',date===today()?'today':'',date===state.selectedDate?'selected':'',mark.kind].filter(Boolean).join(' ');
    return `<button class="${cls}" data-act="selectDay418" data-date="${esc(date)}" aria-label="${esc(date)} ${esc(mark.name||'')}">
      <span class="ob418DayTop"><b>${Number(date.slice(8,10))}</b><i>${esc(mark.label)}</i></span>
      ${mark.name&&mark.kind==='holiday'?`<small class="ob418HolidayName">${esc(mark.name.replace('の日',''))}</small>`:''}
      <span class="ob418Bars">${multis.map(e=>`<em class="ob418PlanBar ${e.type==='camp'?'camp':''} ${date===e.start?'start':''} ${date===(e.end||e.start)?'end':''}">${ob418BarLabel(e,date)}</em>`).join('')}</span>
      <span class="ob418Singles">${singles.map(e=>`<em class="${e.type==='walk'?'walk':''}">${esc((e.title||'予定').slice(0,7))}</em>`).join('')}</span>
      ${ob418EventsOn(date).length>4?`<strong class="ob418More">+${ob418EventsOn(date).length-4}</strong>`:''}
    </button>`;
  }
  function ob418CalendarBoard(){
    const month=state.currentMonth||today().slice(0,7);
    const [y,m]=month.split('-').map(Number);
    return `<section class="ob418Panel ob418CalendarRoot">
      <div class="ob418CalHead">
        <button class="ob418IconBtn" data-act="prevMonth418">‹</button>
        <div><b>${y}.${pad(m)}</b><small>カレンダーがトップ。土日祝・連泊・選択日の予定をここで見る。</small></div>
        <button class="ob418IconBtn" data-act="nextMonth418">›</button>
      </div>
      <div class="ob418CalOps"><button data-act="goToday418">今日</button><button class="primary" data-act="addEvent" data-date="${esc(state.selectedDate||today())}">＋新規予定</button><button data-route="field">散歩マップ</button></div>
      <div class="ob418Week">${['日','月','火','水','木','金','土'].map((w,i)=>`<span class="${i===0?'sun':i===6?'sat':''}">${w}</span>`).join('')}</div>
      <div class="ob418Days">${ob418MonthDays().map(ob418DayCell).join('')}</div>
      <div class="ob418Legend"><span><i class="sat"></i>土曜</span><span><i class="sun"></i>日曜</span><span><i class="holiday"></i>祝日</span><span><i class="bar"></i>連泊・連動予定</span></div>
    </section>`;
  }
  function ob418PlanRow(e,date){
    const a=ob415Attrs(e);
    return `<button class="ob418PlanRow" data-act="viewPlan418" data-id="${esc(e.id)}">
      <span><b>${esc(e.title||'予定')}</b><small>${fmt(e.start)}${e.end&&e.end!==e.start?'〜'+fmt(e.end):''} / ${esc(ob418EventPhase(e,date||e.start))} / ${esc(e.place||'場所未設定')}</small></span>
      <em>${esc(typeName[e.type]||e.type||'予定')} ${ob415PetLabel?`・${esc(ob415PetLabel(a))}`:''}</em>
    </button>`;
  }
  function ob418TodoMemo(date){
    const memos=(state.notes||[]).slice(0,3).map(n=>`<div class="ob418MiniMemo"><b>${esc(n.title)}</b><small>${esc((n.text||'').slice(0,42))}</small></div>`).join('');
    const todos=[];
    const selected=ob418EventsOn(date);
    if(selected.some(e=>e.type==='camp')){
      const undoneShop=(state.shopping||[]).filter(s=>!s.done).length;
      const unloaded=(state.gear||[]).filter(g=>g.status!=='loaded').length;
      todos.push(`買い物 ${undoneShop}件`, `ギア未積込 ${unloaded}件`);
    }
    if(selected.some(e=>e.type==='walk'))todos.push('散歩開始 / 現在地 / ピン');
    return `<div class="ob418TodoMemo"><div><b>ToDo & メモ</b><small>${todos.length?esc(todos.join(' / ')):'この日のメモ確認'}</small></div><button data-act="addNote">メモ追加</button>${memos?`<div class="ob418MemoList">${memos}</div>`:''}</div>`;
  }
  function ob418SelectedPanel(){
    const date=state.selectedDate||today();
    const mark=ob418DayMark(date);
    const list=ob418EventsOn(date);
    return `<section class="ob418Panel"><div class="ob418PanelHead"><span><b>${fmt(date)} ${mark.kind==='holiday'?`祝 ${esc(mark.name)}`:mark.kind==='sat'?'土曜':mark.kind==='sun'?'日曜':''}</b><small>1回タップでこの表示。同じ日付をもう一度タップで予定追加。</small></span><em>${list.length?`${list.length}件`:'空き'}</em></div>${list.length?list.map(e=>ob418PlanRow(e,date)).join(''):`<div class="ob418Empty"><b>予定なし</b><small>この日に追加するなら＋新規予定。記録だけなら記録タブで未確認箱へ。</small></div>`}${ob418TodoMemo(date)}</section>`;
  }
  function ob418TodayNextPanel(){
    const t=ob417TodayEvents();
    const next=ob417NextEvents(5);
    const inbox=(state.records||[]).filter(r=>!r.eventId||r.eventId==='inbox'||r.target==='未確認箱').length;
    return `<section class="ob418Split">
      <div class="ob418Panel"><div class="ob418PanelHead"><span><b>今日</b><small>今見るもの</small></span><em>${t.length||'—'}</em></div>${t.length?t.map(e=>ob418PlanRow(e,today())).join(''):`<div class="ob418Empty"><b>今日の予定なし</b><small>散歩や記録は予定なしでも開始できる。</small></div>`}</div>
      <div class="ob418Panel"><div class="ob418PanelHead"><span><b>次の外出</b><small>近い順</small></span><em>${next.length||'—'}</em></div>${next.length?next.map(e=>ob418PlanRow(e)).join(''):`<div class="ob418Empty"><b>次の外出なし</b><small>＋新規予定で作成。</small></div>`}${inbox?`<button class="ob418Inbox" data-route="memory">未確認記録 ${inbox}件を整理</button>`:''}</div>
    </section>`;
  }
  function ob418NewDock(){
    return `<details class="ob418NewDock"><summary>＋ 新規予定の種類を選ぶ</summary><div class="ob418TemplateGrid">
      ${[
        ['createCampTemplate','キャンプ','泊まり・デイキャンプ・コテージにも展開','幕'],
        ['createHomeWalkTemplate','通常散歩','自宅から歩く','歩'],
        ['createDriveWalkTemplate','ドライブ散歩','車で行ってから歩く','車'],
        ['createEventWithPetTemplate','イベント 同伴あり','コタ連れ・休憩・水・暑さ寒さ','犬'],
        ['createEventNoPetTemplate','イベント 同伴なし','物販・下見・情報収集','人'],
        ['recordInbox','予定なし記録','あとで整理する','箱']
      ].map(([act,title,sub,icon],i)=>`<button class="${i===0?'primary':''}" data-act="${act}"><b>${esc(title)}</b><small>${esc(sub)}</small><em>${icon}</em></button>`).join('')}
    </div></details>`;
  }
  function ob418Calendar(){
    ob415EnsureOutingModel?.();
    return ob416Shell(`<div class="ob418Page">${ob418CalendarBoard()}${ob418TodayNextPanel()}${ob418SelectedPanel()}${ob418NewDock()}</div>`);
  }
  function ob418RouteLikeHero(title,sub,kicker='OUTBASE'){
    return `<section class="ob418Hero"><div><small>${esc(kicker)}</small><h1>${esc(title)}</h1><p>${esc(sub)}</p></div>${ob416Status?ob416Status():''}</section>`;
  }
  function ob418Prep(){
    if(state.prepTab==='route')return ob416Shell(`${routePrepView()}`);
    if(state.prepTab&&state.prepTab!=='overview')return shell(`<div class="ob418Page">${ob416Status?ob416Status():''}<section class="section ob416LegacyTabs"><div class="tabs">${['overview:全体','route:ルート','meals:料理','shopping:買い物','gear:ギア','weather:天気','pets:ペット','timer:タイマー'].map(x=>{const [id,l]=x.split(':');return `<button class="tab ${state.prepTab===id?'active':''}" data-prep="${id}">${l}</button>`}).join('')}</div></section>${prepBody()}</div>`);
    const e=ob415ViewEvent?.()||current();
    const a=ob415Attrs?.(e)||{};
    const items=ob415PrepItems?.()||[];
    return ob416Shell(`<div class="ob418Page">${ob418RouteLikeHero('準備',`${e?.title||'表示中の外出'}に必要な入口だけ。ルートは移動、散歩地図は記録。`,'READY')}
      <section class="ob418Panel"><div class="ob418PanelHead"><span><b>${esc(e?.title||'外出')}</b><small>${esc(e?.place||'場所未設定')} / ${esc(ob415PetLabel?ob415PetLabel(a):'')}</small></span><em>${a.move==='car'?'車移動':'準備'}</em></div><div class="ob418ActionGrid">${items.map((x,i)=>`<button class="${i===0?'primary':''}" ${x.route?`data-prep="${x.route==='route'?'route':x.route}"`:x.act?`data-act="${x.act}"`:''}><b>${esc(x.title||'入口')}</b><small>${esc(x.sub||'')}</small></button>`).join('')}</div></section></div>`);
  }
  function ob418WalkMapPanel(){
    const km=walkDistance(), sec=walkDurationSec(), sp=walkAvgSpeed();
    const pts=(state.walk.track||[]).length;
    const pins=(state.walk.spots||[]).length+(state.mapPins||[]).length;
    return `<section class="ob418WalkMap"><div class="ob418MapHead"><span><b>散歩マップ</b><small>現地・散歩記録用。移動ルートとは別に、歩いた軌跡・現在地・ピン・写真場所を残す。</small></span><em class="${state.walk.active?'live':''}">${state.walk.active?'LIVE':'MAP'}</em></div><div class="ob418MapCanvas"><canvas id="walkMap" width="680" height="360"></canvas>${pts?`<strong>GPS ${pts}点</strong>`:`<strong>散歩開始 / 現在地で記録</strong>`}</div><div class="ob418MapStats"><span><b>${km.toFixed(2)}</b><small>km</small></span><span><b>${fmtDuration(sec)}</b><small>時間</small></span><span><b>${sp.toFixed(1)}</b><small>km/h</small></span><span><b>${pins}</b><small>ピン</small></span></div><div class="ob418MapOps"><button class="primary" data-act="toggleWalk">${state.walk.active?'散歩終了':'散歩開始'}</button><button data-act="gps">現在地</button><button data-act="addPinQuick" data-kind="ピン">ピン</button><button data-act="openMap">Google Map</button></div></section>`;
  }
  function ob418Field(){
    const r=ob415RecordTarget?.();
    return ob416Shell(`<div class="ob418Page">${ob418RouteLikeHero('記録',`保存先：${ob413TargetLabel?.(r)||r?.title||'未確認箱'}`,'RECORD')}${ob418WalkMapPanel()}<section class="ob418Panel"><div class="ob418PanelHead"><span><b>すぐ記録</b><small>話す・撮る・場所・ピン・メモ</small></span><em>3秒</em></div><div class="ob418ActionGrid record">${ob417RecordTools().map((x,i)=>`<button class="${i===0?'primary':''}" data-act="${x.act}"><b>${esc(x.title)}</b><small>${esc(x.sub)}</small></button>`).join('')}</div></section><section class="ob418Panel"><div class="ob418PanelHead"><span><b>散歩セッション</b><small>自宅・ドライブ先・場内/会場を分けて保存</small></span><em>SESSION</em></div><div class="ob418ActionGrid mini">${[
      ['startNormalWalkSession','通常散歩','自宅から歩く'],['startDriveWalkSession','ドライブ散歩','車で行ってから歩く'],['startCampWalkSession','場内/会場散歩','キャンプ・イベント先'],['recordInbox','未確認箱','予定なし記録']
    ].map((x,i)=>`<button class="${i===0?'primary':''}" data-act="${x[0]}"><b>${x[1]}</b><small>${x[2]}</small></button>`).join('')}</div></section>${ob412RecentRecordsHtml?ob412RecentRecordsHtml():''}</div>`);
  }
  function ob418Discover(){
    return ob416Shell(`<div class="ob418Page">${ob418RouteLikeHero('探す','候補探し・下見・保存。予定化する前の調査をここにまとめる。','DISCOVER')}<section class="ob418Panel"><div class="ob418ActionGrid">${[
      ['findCampCandidate','キャンプ場','犬可・温水・4時間以内'],['findWalkCandidate','散歩場所','駐車場・日陰・水辺'],['createEventNoPetTemplate','ペット関連イベント','同伴あり/なしは予定化時に分ける'],['quickRecord','下見記録','駐車場・トイレ・混雑'],['addCandidate','候補追加','あとで比較'],['compareCandidates','上位比較','候補を整理']
    ].map((x,i)=>`<button class="${i===0?'primary':''}" data-act="${x[0]}"><b>${x[1]}</b><small>${x[2]}</small></button>`).join('')}</div></section></div>`);
  }
  function ob418Memory(){
    const rec=(typeof publicRecords==='function')?publicRecords():[];
    return ob416Shell(`<div class="ob418Page">${ob418RouteLikeHero('思い出','記録整理・レビュー・次回改善。現地操作とは分ける。','MEMORY')}<section class="ob418Panel"><div class="ob418PanelHead"><span><b>整理する</b><small>写真・音声・場所・改善</small></span><em>${rec.length}件</em></div><div class="ob418ActionGrid">${[
      ['showLegacyMemory','記録一覧','写真・音声・メモ'],['addReview','次回改善','買い足し・反省'],['recordToPlace','場所メモ','駐車場・トイレ・日陰'],['linkLastToView','最後の記録を関連付け','複数リンク']
    ].map((x,i)=>`<button class="${i===0?'primary':''}" data-act="${x[0]}"><b>${x[1]}</b><small>${x[2]}</small></button>`).join('')}</div></section>${rec.length?`<section class="ob418Panel"><div class="ob418PanelHead"><span><b>最近の記録</b><small>公開候補のみ</small></span><em>${rec.length}</em></div>${rec.slice(0,4).map(fieldRecordCard).join('')}</section>`:''}</div>`);
  }

  calendar = ob418Calendar;
  home = ob418Calendar;
  prep = ob418Prep;
  field = ob418Field;
  discover = ob418Discover;
  memory = ob418Memory;

  const ob418BaseAct = act;
  act = function(a,el){
    if(a==='selectDay418'){
      const date=el.dataset.date||today();
      const now=Date.now();
      const last=state.__calendarLastTap||{};
      rememberScroll();
      if(state.selectedDate===date && last.date===date && now-last.time<1200){
        state.drawer={type:'event',date};
        state.__calendarLastTap={date,time:0};
      }else{
        state.selectedDate=date;state.currentMonth=date.slice(0,7);state.__calendarLastTap={date,time:now};
      }
      save();render(true);return;
    }
    if(a==='prevMonth418'||a==='nextMonth418'){
      rememberScroll();
      const delta=a==='prevMonth418'?-1:1;
      const [y,m]=String(state.currentMonth||today().slice(0,7)).split('-').map(Number);
      const d=new Date(y,m-1+delta,1);
      state.currentMonth=`${d.getFullYear()}-${pad(d.getMonth()+1)}`;
      const selected=state.selectedDate||today();
      if(selected.slice(0,7)!==state.currentMonth)state.selectedDate=iso(d);
      save();render(true);return;
    }
    if(a==='goToday418'){
      rememberScroll();
      state.selectedDate=today();state.currentMonth=today().slice(0,7);save();render(true);return;
    }
    if(a==='viewPlan418'){
      const id=el.dataset.id;
      const e=(state.events||[]).find(x=>x.id===id);
      if(e){ob413SetView?.(id,'plan',{render:false,toast:false});state.currentPlanId=id;if(e.start){state.selectedDate=e.start;state.currentMonth=e.start.slice(0,7)}state.drawer={type:'event',id};save();render(true);}
      return;
    }
    return ob418BaseAct(a,el);
  };

  const ob418BaseBind = bind;
  bind = function(){
    ob418BaseBind();
    // タブ切替だけ先頭へ。それ以外の同一画面操作は位置保持。
    document.querySelectorAll('[data-route]').forEach(el=>el.onclick=()=>{
      const next=el.dataset.route;
      const changed=state.route!==next;
      state.route=next;
      if(next==='prep'){state.prepTab=state.prepTab||'overview';state.stage='prep'}
      if(next==='field')state.stage='field';
      state.routeFocus='';state.__scrollY=changed?0:scrollYNow();
      save();render(!changed);
    });
    ...
    document.querySelectorAll('[data-discover]').forEach(el=>el.onclick=()=>{rememberScroll();state.route='discover';state.discoverTab=el.dataset.discover;save();render(true)});
    document.querySelectorAll('[data-memory]').forEach(el=>el.onclick=()=>{rememberScroll();state.route='memory';state.memoryTab=el.dataset.memory;save();render(true)});
  };

  function ob418FinalAudit(){
    ob415EnsureOutingModel?.();
    const cal=calendar(), fld=field(), pr=prep(), rt=routePrepView?.()||'';
    const checks=[
      ['route approved area kept',/自宅出発/.test(rt)&&/到着/.test(rt)],
      ['calendar is top',/ob418CalendarRoot/.test(cal)&&/今日/.test(cal)&&/次の外出/.test(cal)],
      ['new plan types behind details',/ob418NewDock/.test(cal)&&!/^<[^>]+>.*通常散歩/s.test(cal.slice(0,900))],
      ['weekend holiday marks',/土曜/.test(cal)&&/日曜/.test(cal)&&/祝日/.test(cal)],
      ['linked multi day bar',/ob418PlanBar/.test(cal)],
      ['one tap two tap action',/selectDay418/.test(cal)],
      ['walk map in record',/ob418WalkMap/.test(fld)&&/散歩マップ/.test(fld)],
      ['scroll keep override',/render\(true\)/.test(String(act))&&/changed\?0/.test(String(bind))],
      ['context model remains internal',!!state.context&&Array.isArray(state.sessions)&&Array.isArray(state.links)],
      ['outing attrs remain',!!state.outingAttributes&&!!state.outingTemplates]
    ];
    state.lastAudit0418={version:OUTBASE_RESTORE0418_LOCK.version,time:ob413Now?.()||new Date().toISOString(),checks:checks.map(([name,pass])=>({name,pass:!!pass})),pass:checks.every(x=>x[1]),decision:checks.every(x=>x[1])?'zip_candidate':'stop'};
  }
  ob418FinalAudit();
