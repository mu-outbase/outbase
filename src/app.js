(function(){
  ...
  function bindPlanActions(){
    ...
    document.querySelectorAll('[data-tab-from-switcher]').forEach(el=>el.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();blockUnderlyingNavigation(250);planSheet='';active=el.dataset.tabFromSwitcher||'plan';history.replaceState(null,'',`?tab=${active}&v=clean-v6-library07`);render();window.scrollTo({top:0,behavior:'instant'});}));
...
過去の予定に保存された名前は残ります。`))return;companions=companions.filter(c=>c.id!==row.id);if(planDraft)planDraft.companionIds=planDraft.companionIds.filter(id=>id!==row.id);persistPlans();render();}));
  ...
  function bindPrepActions(){
    ...
    document.querySelectorAll('[data-library-tab]').forEach(el=>el.addEventListener('click',()=>{const sc=document.querySelector('.libraryPageScroll'),top=sc?sc.scrollTop:0;rememberLibraryScroll();libraryTab=el.dataset.libraryTab;libraryScrollPositions[libraryScrollKey()]=top;libraryCloseEditor();persistLibraryTab();render();}));
    document.querySelectorAll('[data-library-gear-view]').forEach(el=>el.addEventListener('click',()=>{const sc=document.querySelector('.libraryPageScroll'),top=sc?sc.scrollTop:0;rememberLibraryScroll();libraryGearView=el.dataset.libraryGearView;libraryScrollPositions[libraryScrollKey()]=top;persistLibraryGearView();libraryCloseEditor();render();}));
    document.querySelectorAll('[data-dashboard-jump]').forEach(el=>el.addEventListener('click',()=>{const sc=document.querySelector('.libraryPageScroll'),top=sc?sc.scrollTop:0;rememberLibraryScroll();libraryGearView=el.dataset.dashboardJump||'dashboard';libraryScrollPositions[libraryScrollKey()]=top;persistLibraryGearView();libraryCloseEditor();render();}));
  ...
  function render(){
    ...
    document.querySelectorAll('.navBtn').forEach(btn=>btn.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();if(anySheetOpen()||Date.now()<modalTapBlockUntil)return;active=btn.dataset.tab;recordSheet='';planSheet='';prepSheet='';prepModuleId='';history.replaceState(null,'',`?tab=${active}&v=clean-v6-library07`);render();window.scrollTo({top:0,behavior:'instant'});}));
    bindPlanActions();
    bindPrepActions();
    bindRecordActions();
