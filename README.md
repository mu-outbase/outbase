...
v140本開発構成の最初の実装として、次のキャンプ準備エンジンを追加した。

- 予約スクショ / PDF / 予約メール / カレンダー予定から候補化
- 候補承認で次のキャンプカード作成
- 買い物・持ち物・コタ用品・反省候補を生成
- リン送信用リストを生成

この段階でもMVP完成とは呼ばない。実機スクショ・実際の予約メール・実際のカレンダー予定で確認する。


## Core02 追加内容

Core02では、Core01で作成した次キャンプカードに対して、天気・人数・コタ同行・献立・過去反省・ギアメモを足し、買い物・持ち物・コタ用品・反省候補を実用化する。

MVP完成ではない。Core02の到達点は「キャンプ前なら使ってもいい」。
「これなら使いたい」はCore03で散歩・当日・日常記録が戻ってから判定する。
