...
(() => {
  ...
  function formatRange(e){const s=new Date(e.start),t=new Date(e.end||e.start);if(e.allDay)return `${s.getMonth()+1}/${s.getDate()} 終日`;return `${s.getMonth()+1}/${s.getDate()} ${pad(s.getHours())}:${pad(s.getMinutes())}〜${t.getMonth()+1}/${t.getDate()} ${pad(t.getHours())}:${pad(t.getMinutes())}`}
  ...
  function renderMultiDayBars(cells){
    ...
    rows.forEach((e,idx)=>{
      ...
      while(cursor<=end){
        ...
        const row=idx%2;
        bars.push(`<div class="multiday-row ${dateKey(cursor)===eventStartDay(e)?'start':'middle'} ${dateKey(segmentEnd)===eventEndDay(e)?'end':''}" style="left:calc((100% / 7) * ${startCol-1} + 3px);width:calc((100% / 7) * ${endCol-startCol} - 6px);top:${58 + week*92 + row*24}px">${dateKey(cursor)===eventStartDay(e)?esc(e.title):''}</div>`);
  ...
  function eventCard(e){const cal=calendarById(e.calendarId);return `<article class="event-card" data-edit="${esc(e.id)}"><h3>${esc(e.title)}</h3><p>${esc(formatRange(e))}</p>${e.place?`<p>${esc(e.place)}</p>`:''}<div class="event-meta"><span class="badge">${esc(cal.name)}</span>${e.repeat!=='none'?`<span class="badge">繰り返し</span>`:''}${e.notify!=='none'?`<span class="badge">通知</span>`:''}</div></article>`}
  ...
  function openSettings(){
    openModal({eyebrow:'CALENDAR',title:'カレンダー設定',body:`
      <div class="settings-list">
        ...
        <section class="settings-block"><h3>将来のアプリ・ウィジェット</h3><p>今日の予定・近日予定・月間マーカーを同じデータから生成できる構造で保持します。</p></section>
      </div>`});
