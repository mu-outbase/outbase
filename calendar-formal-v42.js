...
(() => {
  ...
  calendars=read(CALENDAR_KEY,calendars);
  ...
  function openEvent(id,forcedDate){
    ...
    modal('予定',e?'予定を編集':fmtDate(base),`
      <form id="eventForm" class="event-editor quick-event-editor">
        ...
        <label class="quick-title">
          <span>予定名</span>
          <input name="title" required autofocus autocomplete="off" value="${esc(e?.title||'')}" placeholder="予定名を入力">
        </label>

        <button type="submit" class="quick-save-button">保存</button>

        <details class="editor-advanced">
          <summary>日時・種類などを設定</summary>
          <div class="editor-advanced-body">
            <div class="editor-date-block">
              ...
              <label class="editor-all-day"><input type="checkbox" name="allDay" ${e?.allDay?'checked':''}><span>終日</span></label>
            </div>
            ...
            ${e?'<div class="editor-secondary-actions"><button type="button" id="duplicateEvent">複製</button><button type="button" id="deleteEvent" class="danger">削除</button></div>':''}
          </div>
        </details>
      </form>
    `);
    ...
    $('#addCalendarType')?.addEventListener('click',()=>{
      ...
      const item={id:`custom-${Date.now().toString(36)}`,name:clean};
