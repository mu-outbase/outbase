  ...
  function renderMultiDayBars(cells){
    ...
    rows.forEach((e,idx)=>{
      ...
      while(cursor<=end){
        ...
        const row=idx%2;
        bars.push(`<div class="multiday-row ${dateKey(cursor)===eventStartDay(e)?'start':'middle'} ${dateKey(segmentEnd)===eventEndDay(e)?'end':''}" style="left:calc((100% / 7) * ${startCol-1} + 4px);width:calc((100% / 7) * ${endCol-startCol} - 8px);top:${56 + week*76 + row*22}px">${dateKey(cursor)===eventStartDay(e)?esc(e.title):''}</div>`);
