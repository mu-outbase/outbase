...
(() => {
  ...
  function renderMonth(){
    ...
    return `<section class="calendar-card"><div class="weekdays">${[...'日月火水木金土'].map(x=>`<span>${x}</span>`).join('')}</div><div class="month-grid">${cells().map(d=>{
      ...
      if(m){const st=k===startDay(m),en=k===endDay(m),cls=st&&en?'single':st?'start':en?'end':'middle';seg=`<div class="multi-lane"><span class="segment ${cls}">${st?esc(m.title):''}</span></div>`}
      return `<button class="day-cell ${d.getMonth()!==cur?'outside':''} ${k===state.selected?'selected':''} ${k===today?'today':''}" data-date="${k}"><strong>${d.getDate()}</strong>${seg}<div class="single-lane">${singles.slice(0,2).map(e=>`<span class="mini-event">${esc(e.title)}</span>`).join('')}</div></button>`
    }).join('')}</div></section>`
  }
  ...
  $('#addEventBtn').onclick=()=>openEvent();$('#quickAddBtn').onclick=()=>openEvent();$('#navAdd').onclick=()=>openEvent();$('#settingsBtn').onclick=openSettings;$('#filterBtn').onclick=()=>$('#filterPanel').classList.toggle('hidden');$('#closeModal').onclick=close;$('#modal').onclick=e=>{if(e.target.id==='modal')close()};
