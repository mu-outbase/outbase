...
(() => {
      ...
      const cellsHtml=days.map((d,index)=>{
        ...
        return `<button class="day-cell ${d.getMonth()!==cur?'outside':''} ${k===state.selected?'selected':''} ${k===today?'today':''} ${barCounts[index]>=2?'has-two-bars':''}" data-date="${k}">
          ...
          <span class="day-cell-events">
            ${singles.slice(0,visibleLimit).map(e=>`<span class="cell-event tone-${esc(e.calendarId)}">${esc(e.title)}</span>`).join('')}
            ${singles.length>visibleLimit?`<span class="cell-event cell-more">ほか${singles.length-visibleLimit}件</span>`:''}
          </span>
        </button>`;
      }).join('');
      const barsHtml=bars.map(bar=>`<span class="week-bar tone-${esc(bar.event.calendarId)}" style="grid-column:${bar.startCol+1}/${bar.endCol+2};grid-row:${bar.lane+1}" title="${esc(bar.event.title)}">${bar.showTitle?esc(bar.event.title):''}</span>`).join('');
  ...
  function openEvent(id){
    ...
    const selectedCalendar=e?.calendarId||'activity';
    const startValue=e?.start||base+'T09:00';
    const endValue=e?.end||base+'T10:00';

    modal('予定',e?'予定を編集':'予定を追加',`
      <form id="eventForm" class="event-editor">
        <input type="hidden" name="id" value="${esc(e?.id||'')}">

        <section class="editor-primary">
          <label class="editor-title-field">
            <span>タイトル</span>
            <input name="title" required autocomplete="off" value="${esc(e?.title||'')}" placeholder="予定名">
          </label>

          <div class="editor-date-block">
            <label>
              <span>開始</span>
              <input type="datetime-local" name="start" required value="${esc(startValue)}">
            </label>
            <label>
              <span>終了</span>
              <input type="datetime-local" name="end" required value="${esc(endValue)}">
            </label>
            <label class="editor-all-day">
              <input type="checkbox" name="allDay" ${e?.allDay?'checked':''}>
              <span>終日</span>
            </label>
          </div>
          ...
          <fieldset class="editor-chip-group">
            ...
            <div class="editor-chips" data-calendar-chips>
              ${calendars.map(c=>`<button type="button" class="editor-chip ${selectedCalendar===c.id?'active':''}" data-calendar-value="${c.id}">${esc(c.name)}</button>`).join('')}
            </div>
          </fieldset>
          ...
          <label>
            <span>場所</span>
            <input name="place" value="${esc(e?.place||'')}" placeholder="場所">
          </label>

          <label>
            <span>メモ</span>
            <textarea name="memo" rows="3" placeholder="メモ">${esc(e?.memo||'')}</textarea>
          </label>
        </section>
        ...
        <details class="editor-advanced">
          ...
          <div class="editor-advanced-body">
            ...
            <div class="form-grid two">
              <label>
                <span>間隔</span>
                <input type="number" min="1" name="interval" value="${e?.repeat?.interval||1}">
              </label>
              <label>
                <span>繰り返し終了</span>
                <input type="date" name="until" value="${esc(e?.repeat?.until||'')}">
              </label>
            </div>
          </div>
        </details>
    ...
    f.onsubmit=ev=>{
      ...
      const row={
        ...
        calendarId:fd.get('calendarId'),
        participants:String(fd.get('participants')||'').split(/[、,]/).map(x=>x.trim()).filter(Boolean),
        place:fd.get('place'),
        memo:fd.get('memo'),
        repeat:{freq:fd.get('freq'),interval:Number(fd.get('interval')||1),until:fd.get('until')},
    ...
    $('#deleteEvent')?.addEventListener('click',()=>{
      if(confirm('削除しますか？')){
        saveEvents(events().filter(x=>x.id!==e.id));
        close();
        render();
      }
    });
    ...
    $('#duplicateEvent')?.addEventListener('click',()=>{
      saveEvents([...events(),{...e,id:uid(),title:e.title+'（コピー）'}]);
      close();
      render();
    });
  }
