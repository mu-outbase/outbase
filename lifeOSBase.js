...
(function(){
  ...
  function setReady(){ document.body.classList.add('outbase-life-ready'); }
  function getPage(id){ return document.getElementById(id); }
  function isVisible(id){ const el = getPage(id); return !!el && !el.classList.contains('hidden'); }

  function hideAllPages(){ document.querySelectorAll('.page').forEach(page => page.classList.add('hidden')); }
  function hideLifePages(){ Object.values(PAGES).filter(id => id !== 'homePage').forEach(id => { const page = getPage(id); if(page) page.classList.add('hidden'); }); }
  ...
  function readProjects(){
    try{ const raw = localStorage.getItem('outbase_camp_projects_v1'); return raw ? JSON.parse(raw) : []; }
    catch(error){ return []; }
  }
  ...
  function latestRecord(list){
    return (list || []).slice().sort((a,b)=> new Date(b.date || b.createdAt || b.created_at || 0) - new Date(a.date || a.createdAt || a.created_at || 0))[0] || null;
  }
  ...
  async function renderHome(force){
    ...
    try{
      ...
      if(!home) return;
      home.classList.remove('outbase-life-ready-page');
      home.innerHTML = buildHome(await collect());
      home.classList.add('outbase-life-ready-page');
      if(currentRoute === 'home' || force) home.classList.remove('hidden');
      setReady();
  ...
  async function renderRecord(){
    ...
    page.innerHTML = `<div class="life-v208-inner">${hero('RECORD','写真・音声・メモを<br>迷わず残す。','記録入力はここに集約。ホームには増やさない。',false)}
      ...
      <section class="life-card"><div class="life-list">
        ...
        <div class="life-row"><div class="life-row-icon">🎤</div><div><div class="life-row-title">音声メモ</div><div class="life-row-meta">散歩中は長押し音声、通常記録はここから。</div></div></div>
  ...
  async function showLifePage(key){
    addStyle();
    currentRoute = key === 'home' ? 'home' : key;
    hideAllPages();
    if(key === 'home'){
  ...
  function patchEntrances(){
    if(typeof window.showPage === 'function' && !window.showPage.__lifeV208Patched){
      ...
      window.showPage = function(pageId){
        ...
        currentRoute = pageId || 'external';
        const result = original.apply(this, arguments);
        hideLifePages();
        const home = getPage('homePage');
        if(home && pageId !== 'homePage') home.classList.add('hidden');
        setReady();
        return result;
      };
  ...
  async function setup(){
    ...
    patchEntrances();
    observeHome();
    if(isVisible('walkPage')){
      currentRoute = 'walkPage';
      hideLifePages();
      const home = getPage('homePage');
